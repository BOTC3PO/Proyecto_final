# Idiomas — Alemán — particulas-modales (cuestionario)

> Corregido a partir del borrador (`../../../_borradores-gemma/idiomas-extranjeros/aleman/particulas-modales/cuestionario_crudo.md`): arreglo mecánico (comillas mal cerradas, pasos sin comillas, `---` sueltos), sin cambiar ninguna respuesta ni explicación original.

---

### 1 — Partikeln in einer Antwort  
```yaml
metadata:
  materia: "idiomas-extranjeros/aleman"
  tema: "partikeln-modales"
  nivel: "C1"
  tags: ["antworten", "doch"]
pasos:
  - "Identifica el contexto de la oración y el rol de la partícula."
  - "Reemplaza el espacio con una partícula que exprese duda o contradicción."
tipo: completar
respuestas_validas:
  - "Doch"
  - "DOCH"
enunciado: "Er hat das Projekt nicht abgeschlossen, ______? Ich denke, er hat es nur vergessen."
explicacion: "La partícula 'doch' se usa para expresar duda o contradicción ante una afirmación. Aquí se cuestiona la negativa de que el proyecto no se completó."
```



### 2 — Partikeln en un diálogo  
```yaml
metadata:
  materia: "idiomas-extranjeros/aleman"
  tema: "partikeln-modales"
  nivel: "C1"
  tags: ["dialog", "ja"]
pasos:
  - "Reconoce el tono de la oración y el propósito de la partícula."
  - "Elige la partícula que refuerce una afirmación en un contexto informal."
tipo: completar
respuestas_validas:
  - "JA"
  - "Ja"
enunciado: "Ich habe das Buch noch nicht gelesen. ______, ich bin gerade damit beschäftigt!"
explicacion: "'Ja' se usa aquí para reforzar una afirmación en un contexto conversacional, indicando que la acción está en curso."
```



### 3 — Partikeln de duda  
```yaml
metadata:
  materia: "idiomas-extranjeros/aleman"
  tema: "partikeln-modales"
  nivel: "C1"
  tags: ["zweifeln", "vielleicht"]
pasos:
  - "Analiza si la oración expresa incertidumbre o posibilidad."
  - "Elige la partícula que indique una suposición no confirmada."
tipo: completar
respuestas_validas:
  - "Vielleicht"
  - "VIELLEICHT"
enunciado: "Er kommt ______ später. Der Zug war zu voll, um pünktlich anzukommen."
explicacion: "'Vielleicht' expresa posibilidad o incertidumbre sobre el horario del tren."
```



### 4 — Partikeln de enfoque  
```yaml
metadata:
  materia: "idiomas-extranjeros/aleman"
  tema: "partikeln-modales"
  nivel: "C1"
  tags: ["betonung", "schon"]
pasos:
  - "Identifica si la partícula enfatiza una acción ya realizada."
  - "Elige 'schon' para marcar un tiempo anterior al esperado."
tipo: completar
respuestas_validas:
  - "Schon"
  - "SCHON"
enunciado: "Er hat die Prüfung ______ bestanden. Ich dachte, er hätte noch mehr Zeit gebraucht!"
explicacion: "'Schon' enfatiza que la acción (pasar el examen) ocurrió antes de lo esperado."
```



### 5 — Partikeln en una negación  
```yaml
metadata:
  materia: "idiomas-extranjeros/aleman"
  tema: "partikeln-modales"
  nivel: "C1"
  tags: ["negation", "gar nicht"]
pasos:
  - "Reconoce si la partícula intensifica una negación."
  - "Elige 'gar nicht' para enfatizar que algo no ocurre en absoluto."
tipo: completar
respuestas_validas:
  - "Gar nicht"
  - "GAR NICHT"
enunciado: "Er ist ______ interessiert an dem Thema. Er hat es nie erwähnt!"
explicacion: "'Gar nicht' intensifica la negación, indicando total desinterés."
```



### 6 — Partikeln de corrección  
```yaml
metadata:
  materia: "idiomas-extranjeros/aleman"
  tema: "partikeln-modales"
  nivel: "C1"
  tags: ["korrektur", "doch"]
pasos:
  - "Analiza si la partícula corrige una suposición incorrecta."
  - "Elige 'doch' para contradecir una afirmación previa."
tipo: completar
respuestas_validas:
  - "Doch"
  - "DOCH"
enunciado: "Du hast es nicht verstanden, ______? Ich habe es dir doch erklärt!"
explicacion: "'Doch' se usa para contradecir la negativa ('nicht verstanden') y reafirmar una explicación previa."
```



### 7 — Partikeln en un consejo  
```yaml
metadata:
  materia: "idiomas-extranjeros/aleman"
  tema: "partikeln-modales"
  nivel: "C1"
  tags: ["rat", "lieber"]
pasos:
  - "Reconoce si la partícula sugiere una alternativa preferida."
  - "Elige 'lieber' para expresar una opción más deseada."
tipo: completar
respuestas_validas:
  - "Lieber"
  - "LIEBER"
enunciado: "Ich gehe ______ mit dem Auto. Es ist schneller als zu Fuß!"
explicacion: "'Lieber' expresa preferencia por el medio de transporte (coche) debido a su rapidez."
```



### 8 — Partikeln en una afirmación  
```yaml
metadata:
  materia: "idiomas-extranjeros/aleman"
  tema: "partikeln-modales"
  nivel: "C1"
  tags: ["bestätigung", "tatsächlich"]
pasos:
  - "Identifica si la partícula confirma una afirmación."
  - "Elige 'tatsächlich' para reforzar un hecho real."
tipo: completar
respuestas_validas:
  - "Tatsächlich"
  - "TATSÄCHLICH"
enunciado: "Er ist ______ krank. Der Arzt hat es bestätigt!"
explicacion: "'Tatsächlich' confirma que la enfermedad es real, respaldada por el médico."
```



### 9 — Partikeln de sorpresa  
```yaml
metadata:
  materia: "idiomas-extranjeros/aleman"
  tema: "partikeln-modales"
  nivel: "C1"
  tags: ["überraschung", "wohl"]
pasos:
  - "Analiza si la partícula expresa sorpresa o expectativa."
  - "Elige 'wohl' para sugerir una probabilidad no confirmada."
tipo: completar
respuestas_validas:
  - "Wohl"
  - "WOHL"
enunciado: "Er ist ______ nicht mehr im Büro. Ich habe ihn heute nicht gesehen!"
explicacion: "'Wohl' expresa una suposición sobre la ausencia, basada en la observación de hoy."
```



### 10 — Partikeln en un comentario  
```yaml
metadata:
  materia: "idiomas-extranjeros/aleman"
  tema: "partikeln-modales"
  nivel: "C1"
  tags: ["kommentar", "schon"]
pasos:
  - "Reconoce si la partícula indica que algo ya ha ocurrido."
  - "Elige 'schon' para marcar un tiempo anterior al esperado."
tipo: completar
respuestas_validas:
  - "Schon"
  - "SCHON"
enunciado: "Er hat die Nachricht ______ erhalten. Warum hast du sie mir nicht gesagt?"
explicacion: "'Schon' enfatiza que la noticia llegó antes de lo esperado, generando sorpresa."
```



### 11 — Partikeln en una negación intensa  
```yaml
metadata:
  materia: "idiomas-extranjeros/aleman"
  tema: "partikeln-modales"
  nivel: "C1"
  tags: ["negation", "gar nicht"]
pasos:
  - "Identifica si la partícula intensifica una negación."
  - "Elige 'gar nicht' para enfatizar que algo no ocurre en absoluto."
tipo: completar
respuestas_validas:
  - "Gar nicht"
  - "GAR NICHT"
enunciado: "Er ist ______ zufrieden mit dem Ergebnis. Er hat es nie erwähnt!"
explicacion: "'Gar nicht' intensifica la negación, indicando total insatisfacción."
```



### 12 — Partikeln en una duda  
```
metadata:  
  materia: "aleman"  
  tema: "partikeln-modales"  
  nivel: "C1"  
  tags: ["noch", "nicht"]  
pasos:  
  - "Identificar la partícula que completa correctamente el sentido de negación en el subordinado."  
explicacion: "La partícula 'schon' se usa en oraciones negativas para enfatizar la sorpresa o la novedad. En este contexto, 'schon' resalta que la acción ya ocurrió aunque no se esperaba."  
enunciado: "Er hat es ______ verstanden. Warum hast du nicht nachgefragt?"  
tipo: completar  
respuestas_validas:
  - "Schon"
  - "SCHON"
```

### 13 — Partikeln en una afirmación  
```
metadata:  
  materia: "aleman"  
  tema: "partikeln-modales"  
  nivel: "C1"  
  tags: ["müssen", "konjunktiv"]  
pasos:  
  - "Elegir el modo verbal correcto para expresar una necesidad hipotética."  
explicacion: "El konjunktiv II de 'müssen' se forma con 'müsse' cuando se habla de obligaciones imaginarias o deseos. Aquí, expresa una posibilidad no realizada."  
enunciado: "Wenn ich mehr Zeit ______, würde ich es sofort tun."  
tipo: completar  
respuestas_validas:
  - "Hätte"
  - "HATTE"
```

### 14 — Partikeln en un consejo  
```
metadata:  
  materia: "aleman"  
  tema: "partikeln-modales"  
  nivel: "C1"  
  tags: ["nur", "dialogo"]  
pasos:  
  - "Seleccionar la partícula que completa el sentido de limitación en el diálogo."  
explicacion: "'Nur' se usa para restringir una acción o estado, enfatizando exclusividad. En este contexto, expresa que no hay otras opciones posibles."  
enunciado: "Ich glaube nicht, dass er ______ in der Lage ist, das zu bewältigen."  
tipo: completar  
respuestas_validas:
  - "Nur"
  - "NUR"
```

### 15 — Partikeln en una negación intensa  
```
metadata:  
  materia: "aleman"  
  tema: "partikeln-modales"  
  nivel: "C1"  
  tags: ["dürfen", "bedingung"]  
pasos:  
  - "Identificar el modal correcto para expresar permiso en una condición."  
explicacion: "'Dürfen' se usa en condiciones hipotéticas con 'wenn... dann...' para indicar autorización. Aquí, el tono es especulativo y no realista."  
enunciado: "Wenn du nicht so müde ______, könnten wir noch später gehen."  
tipo: completar  
respuestas_validas:
  - "Wärst"
  - "WARST"
```

### 16 — Partikeln en una duda  
```
metadata:  
  materia: "aleman"  
  tema: "partikeln-modales"  
  nivel: "C1"  
  tags: ["schon", "duda"]  
pasos:  
  - "Escoger la partícula que intensifica la sorpresa en una oración interrogativa."  
explicacion: "'Schon' en preguntas subraya lo inesperado. En este caso, enfatiza que el sujeto ya lo sabía antes de la pregunta."  
enunciado: "Hast du das wirklich ______ gewusst? Ich dachte, du hättest es erst heute erfahren!"  
tipo: completar  
respuestas_validas:
  - "Schon"
  - "SCHON"
```

### 17 — Partikeln en una afirmación  
```
metadata:  
  materia: "aleman"  
  tema: "partikeln-modales"  
  nivel: "C1"  
  tags: ["sollen", "prohibicion"]  
pasos:  
  - "Determinar el modal que expresa una prohibición no cumplida."  
explicacion: "'Sollen' en konjunktiv II indica una obligación no respetada. Aquí, se usa para criticar una acción omitida."  
enunciado: "Du ______ das nicht tun! Warum hast du es dennoch gemacht?"  
tipo: completar  
respuestas_validas:
  - "Solltest"
  - "SOULTEST"
```

### 18 — Partikeln en un consejo  
```
metadata:  
  materia: "aleman"  
  tema: "partikeln-modales"  
  nivel: "C1"  
  tags: ["denn", "enfatizacion"]  
pasos:  
  - "Elegir la partícula que reforza el sentido de causalidad en una oración."  
explicacion: "'Denn' se usa para conectar causas y efectos, especialmente cuando se explica una razón posterior. Aquí, justifica la acción pasada."  
enunciado: "Er hat es nicht getan. ______, er war einfach nicht bereit dazu."  
tipo: completar  
respuestas_validas:
  - "Denn"
  - "DENN"
```

### 19 — Partikeln en una negación intensa  
```
metadata:  
  materia: "aleman"  
  tema: "partikeln-modales"  
  nivel: "C1"  
  tags: ["müssen", "obligacion"]  
pasos:  
  - "Identificar el modal que expresa una obligación real."  
explicacion: "'Müssen' en presente indica una responsabilidad actual. En este contexto, se refiere a un deber inmediato y necesario."  
enunciado: "Ich ______ das Projekt noch heute abschließen. Der Chef hat es verlangt!"  
tipo: completar  
respuestas_validas:
  - "Muss"
  - "MUSS"
```

### 20 — Partikeln en una duda  
```
metadata:  
  materia: "aleman"  
  tema: "partikeln-modales"  
  nivel: "C1"  
  tags: ["gar", "negacion"]  
pasos:  
  - "Seleccionar la partícula que intensifica una negativa en un contexto emocional."  
explicacion: "'Gar' se usa para enfatizar negativas, especialmente cuando hay sorpresa o frustración. Aquí, subraya que el sujeto negó algo incluso aunque era evidente."  
enunciado: "Er hat es ______ verstanden! Warum erzählt er es dann niemandem?"  
tipo: completar  
respuestas_validas:
  - "Gar"
  - "GAR"
```

### 21 — Partikeln en una afirmación  
```
metadata:  
  materia: "aleman"  
  tema: "partikeln-modales"  
  nivel: "C1"  
  tags: ["dürfen", "duda"]  
pasos:  
  - "Elegir el modal que expresa permiso no concedido en una situación hipotética."  
explicacion: "'Dürfen' en konjunktiv II indica un permiso que no se dio. Aquí, se habla de algo que podría haber ocurrido si hubiera tenido autorización."  
enunciado: "Wenn du ______, hättest du es schon längst getan!"  
tipo: completar  
respuestas_validas:
  - "Dürftest"
  - "DÜRFTEST"
```

### 22 — Partikeln en un consejo  
```
metadata:  
  materia: "aleman"  
  tema: "partikeln-modales"  
  nivel: "C1"  
  tags: ["noch", "repeticion"]  
pasos:  
  - "Identificar la partícula que resalta una repetición no necesaria."  
explicacion: "'Noch' se usa para enfatizar que algo ya ocurrió y no se necesita hacer de nuevo. En este contexto, indica frustración por un acto innecesario."  
enunciado: "Du bist ______ aufgestanden! Du hast doch schon genug getan!"  
tipo: completar  
respuestas_validas:
  - "Noch"
  - "NOCH"
```

### 23 — Partikeln en una negación intensa  
```
metadata:  
  materia: "aleman"  
  tema: "partikeln-modales"  
  nivel: "C1"  
  tags: ["sollen", "responsabilidad"]  
pasos:  
  - "Seleccionar el modal que expresa una expectativa no cumplida."  
explicacion: "'Sollen' en konjunktiv II se usa para criticar una acción omitida, especialmente cuando hay una responsabilidad clara. Aquí, indica decepción por la falta de cumplimiento."  
enunciado: "Er ______ es nicht tun! Warum hat er es dann nicht gemacht?"  
tipo: completar  
respuestas_validas:
  - "Sollte"
  - "SOULTE"
```

### 24 — Partikeln en una duda  
```
metadata:  
  materia: "aleman"  
  tema: "partikeln-modales"  
  nivel: "C1"  
  tags: ["ja", "confirmacion"]  
pasos:  
  - "Elegir la partícula que subraya una afirmación en un contexto emocional."  
explicacion: "'Ja' como partícula se usa para enfatizar confirmaciones, especialmente cuando hay sorpresa o alivio. Aquí, confirma algo que ya era evidente pero no esperado."  
enunciado: "Er hat es ______ verstanden! Warum hat er dann so lange gezögert?"  
tipo: completar  
respuestas_validas:
  - "Ja"
  - "JA"
```

### 25 — Partikeln en una afirmación  
```
metadata:  
  materia: "aleman"  
  tema: "partikeln-modales"  
  nivel: "C1"  
  tags: ["können", "posibilidad"]  
pasos:  
  - "Identificar el modal que expresa una posibilidad no realizada en el pasado."  
explicacion: "'Können' en konjunktiv II indica algo que era posible pero no ocurrió. Aquí, se habla de un evento hipotético que podría haber tenido lugar si hubiera habido condiciones adecuadas."  
enunciado: "Wenn du ______, hättest du es sicher geschafft!"  
tipo: completar  
respuestas_validas:
  - "Konntest"
  - "KONNTEST"```
