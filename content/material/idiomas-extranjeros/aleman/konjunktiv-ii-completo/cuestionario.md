# Idiomas — Alemán — konjunktiv-ii-completo (cuestionario)

> Corregido a partir del borrador (`../../../_borradores-gemma/idiomas-extranjeros/aleman/konjunktiv-ii-completo/cuestionario_crudo.md`): arreglo mecánico + 4 bloques sin `enunciado:` — agregado. El bloque sobre 'kennen' (conocer) pedía como respuesta 'käme' (que en realidad es de 'kommen', venir) — corregido a 'kennte', la forma real de 'kennen'.

---

### 1 — Konjunktiv II en contexto de deseo  
```yaml
metadata:
  materia: "idiomas-extranjeros/aleman"
  tema: "konjunktiv-ii-completo"
  nivel: "B2"
  tags: ["deseos", "präteritum"]
pasos:
  - "Identifica la forma correcta de 'haber' en Konjunktiv II."
  - "Completa el hueco con la conjugación adecuada del verbo."
respuestas_validas:
  - "Hätte"
tipo: completar
enunciado: "Wenn ich ___ mehr Zeit hätte, würde ich reisen."
explicacion: "El Konjunktiv II de 'haben' en este contexto es 'hätte', que se usa para expresar deseos hipotéticos. La forma präteritum del verbo 'haben' (haben → hatte) se emplea aquí."
```




### 2 — Elección de la forma correcta  
```yaml
metadata:
  materia: "idiomas-extranjeros/aleman"
  tema: "konjunktiv-ii-completo"
  nivel: "B2"
  tags: ["opciones", "würde"]
pasos:
  - "Reconoce la forma correcta de 'gehen' en Konjunktiv II."
  - "Selecciona la opción que usa el verbo en el tiempo adecuado."
respuesta: "würde gehen"
opciones_explicitas:
  - "gehe"
  - "würde gehen"
  - "ginge"
tipo: mc
enunciado: "Welche Form ist korrekt? 'Ich ___ mitkommen.'"
explicacion: "La forma correcta es 'würde gehen', ya que el Konjunktiv II con 'würde' se usa para expresar acciones hipotéticas en el presente."
```




### 3 — Completar un deseo  
```yaml
metadata:
  materia: "idiomas-extranjeros/aleman"
  tema: "konjunktiv-ii-completo"
  nivel: "B2"
  tags: ["deseos", "präteritum"]
pasos:
  - "Identifica la conjugación de 'werden' en Konjunktiv II."
  - "Completa el hueco con la forma correcta del verbo."
respuestas_validas:
  - "Würde werden"
tipo: completar
enunciado: "Wenn ich ___ reich wäre, würde ich alles kaufen."
explicacion: "'Werden' en Konjunktiv II es 'würde werden', que se usa para expresar deseos hipotéticos. La forma präteritum no se aplica aquí porque el verbo principal ya está en Konjunktiv II (señalado por 'würde')."
```




### 4 — Elección de la opción correcta  
```yaml
metadata:
  materia: "idiomas-extranjeros/aleman"
  tema: "konjunktiv-ii-completo"
  nivel: "B2"
  tags: ["opciones", "hypothetische"]
pasos:
  - "Selecciona la forma correcta de 'schwimmen' en Konjunktiv II."
  - "Asegúrate de que el verbo esté en el tiempo adecuado para una hipótesis."
respuesta: "würde schwimmen"
opciones_explicitas:
  - "schwimme"
  - "würde schwimmen"
  - "schwämme"
tipo: mc
enunciado: "Welche Form ist korrekt? 'Er ___ im See.'"  
explicacion: "'Würde schwimmen' es la forma correcta, ya que el Konjunktiv II con 'würde' se usa para expresar acciones hipotéticas en el presente."
```




### 5 — Completar un contexto de sugerencia  
```yaml
metadata:
  materia: "idiomas-extranjeros/aleman"
  tema: "konjunktiv-ii-completo"
  nivel: "B2"
  tags: ["sugerencias", "würde"]
pasos:
  - "Identifica la forma correcta de 'essen' en Konjunktiv II."
  - "Completa el hueco con la conjugación adecuada del verbo."
respuestas_validas:
  - "Würde essen"
tipo: completar
enunciado: "Du könntest ___ zu Hause bleiben, wenn es regnet."  
explicacion: "'Essen' en Konjunktiv II es 'würde essen', que se usa para sugerir acciones hipotéticas. La forma präteritum no se aplica aquí porque el verbo principal ya está en Konjunktiv II (señalado por 'könntest')."
```




### 6 — Elección de la opción correcta  
```yaml
metadata:
  materia: "idiomas-extranjeros/aleman"
  tema: "konjunktiv-ii-completo"
  nivel: "B2"
  tags: ["opciones", "hypothetische"]
pasos:
  - "Selecciona la forma correcta de 'fahren' en Konjunktiv II."
  - "Asegúrate de que el verbo esté en el tiempo adecuado para una hipótesis."
respuesta: "würde fahren"
opciones_explicitas:
  - "fahre"
  - "würde fahren"
  - "fuhr"
tipo: mc
enunciado: "Welche Form ist korrekt? 'Ich ___ mit dem Auto.'"  
explicacion: "'Würde fahren' es la forma correcta, ya que el Konjunktiv II con 'würde' se usa para expresar acciones hipotéticas en el presente."
```




### 7 — Completar un contexto de deseo  
```yaml
metadata:
  materia: "idiomas-extranjeros/aleman"
  tema: "konjunktiv-ii-completo"
  nivel: "B2"
  tags: ["deseos", "präteritum"]
pasos:
  - "Identifica la conjugación de 'sein' en Konjunktiv II."
  - "Completa el hueco con la forma correcta del verbo."
respuestas_validas:
  - "Wäre"
tipo: completar
enunciado: "Wenn ich ___ glücklich wäre, würde ich lachen."  
explicacion: "'Sein' en Konjunktiv II es 'wäre', que se usa para expresar deseos hipotéticos. La forma präteritum no se aplica aquí porque el verbo principal ya está en Konjunktiv II (señalado por 'würde')."
```




### 8 — Elección de la opción correcta  
```yaml
metadata:
  materia: "idiomas-extranjeros/aleman"
  tema: "konjunktiv-ii-completo"
  nivel: "B2"
  tags: ["opciones", "hypothetische"]
pasos:
  - "Selecciona la forma correcta de 'trinken' en Konjunktiv II."
  - "Asegúrate de que el verbo esté en el tiempo adecuado para una hipótesis."
respuesta: "würde trinken"
opciones_explicitas:
  - "trinke"
  - "würde trinken"
  - "trank"
tipo: mc
enunciado: "Welche Form ist korrekt? 'Er ___ Wasser.'"  
explicacion: "'Würde trinken' es la forma correcta, ya que el Konjunktiv II con 'würde' se usa para expresar acciones hipotéticas en el presente."
```




### 9 — Completar un contexto de sugerencia  
```yaml
metadata:
  materia: "idiomas-extranjeros/aleman"
  tema: "konjunktiv-ii-completo"
  nivel: "B2"
  tags: ["sugerencias", "würde"]
pasos:
  - "Identifica la forma correcta de 'schlafen' en Konjunktiv II."
  - "Completa el hueco con la conjugación adecuada del verbo."
respuestas_validas:
  - "Würde schlafen"
tipo: completar
enunciado: "Du könntest ___ in der Bibliothek, wenn es ruhig ist."  
explicacion: "'Schlafen' en Konjunktiv II es 'würde schlafen', que se usa para sugerir acciones hipotéticas. La forma präteritum no se aplica aquí porque el verbo principal ya está en Konjunktiv II (señalado por 'könntest')."
```




### 10 — Elección de la opción correcta  
```yaml
metadata:
  materia: "idiomas-extranjeros/aleman"
  tema: "konjunktiv-ii-completo"
  nivel: "B2"
  tags: ["opciones", "hypothetische"]
pasos:
  - "Selecciona la forma correcta de 'rennen' en Konjunktiv II."
  - "Asegúrate de que el verbo esté en el tiempo adecuado para una hipótesis."
respuesta: "würde rennen"
opciones_explicitas:
  - "renne"
  - "würde rennen"
  - "rannte"
tipo: mc
enunciado: "Welche Form ist korrekt? 'Ich ___ um die Ecke.'"  
explicacion: "'Würde rennen' es la forma correcta, ya que el Konjunktiv II con 'würde' se usa para expresar acciones hipotéticas en el presente."
```




### 11 — Completar un contexto de deseo  
```yaml
metadata:
  materia: "idiomas-extranjeros/aleman"
  tema: "konjunktiv-ii-completo"
  nivel: "B2"
  tags: ["deseos", "präteritum"]
pasos:
  - "Identifica la conjugación de 'haben' en Konjunktiv II."
  - "Completa el hueco con la forma correcta del verbo."
respuestas_validas:
  - "Hätte"
tipo: completar
enunciado: "Wenn ich ___ mehr Geld hätte, würde ich reisen."  
explicacion: "'Haben' en Konjunktiv II es 'hätte', que se usa para expresar deseos hipotéticos. La forma präteritum no se aplica aquí porque el verbo principal ya está en Konjunktiv II (señalado por 'würde')."
```




### 12 — Elección de la opción correcta  
```yaml
metadata:
  materia: "idiomas-extranjeros/aleman"
  tema: "konjunktiv-ii-completo"
  nivel: "B2"
  tags: ["opciones", "hypothetische"]
pasos:
  - "Selecciona la forma correcta de 'schreiben' en Konjunktiv II."
  - "Asegúrate de que el verbo esté en el tiempo adecuado para una hipótesis."
respuesta: "würde schreiben"
opciones_explicitas:
  - "schreibe"
  - "würde schreiben"
  - "schrieb"
tipo: mc
enunciado: "Welche Form ist korrekt? 'Er ___ einen Brief.'"  
explicacion: "'Würde schreiben' es la forma correcta, ya que el Konjunktiv II con 'würde' se usa para expresar acciones hipotéticas en el presente."
```




### 13 — Completar un contexto de sugerencia  
```yaml
metadata:
  materia: "idiomas-extranjeros/aleman"
  tema: "konjunktiv-ii-completo"
  nivel: "B2"
  tags: ["sugerencias", "würde"]
pasos:
  - "Identifica la forma correcta de 'machen' en Konjunktiv II."
  - "Completa el hueco con la conjugación adecuada del verbo."
respuestas_validas:
  - "Würde machen"
tipo: completar
enunciado: "Du könntest ___ eine Pause, wenn du müde bist."  
explicacion: "'Machen' en Konjunktiv II es 'würde machen', que se usa para sugerir acciones hipotéticas. La forma präteritum no se aplica aquí porque el verbo principal ya está en Konjunktiv II (señalado por 'könntest')."
```




### 14 — Elección de la opción correcta  
```yaml
metadata:
  materia: "idiomas-extranjeros/aleman"
  tema: "konjunktiv-ii-completo"
  nivel: "B2"
  tags: ["opciones", "hypothetische"]
pasos:
  - "Selecciona la forma correcta de 'fliegen' en Konjunktiv II."
  - "Asegúrate de que el verbo esté en el tiempo adecuado para una hipótesis."
respuesta: "würde fliegen"
opciones_explicitas:
  - "fliege"
  - "würde fliegen"
  - "flog"
tipo: mc
enunciado: "Welche Form ist korrekt? 'Ich ___ mit dem Flugzeug.'"  
explicacion: "'Würde fliegen' es la forma correcta, ya que el Konjunktiv II con 'würde' se usa para expresar acciones hipotéticas en el presente."
```




### 15 — Completar un contexto de deseo  
```yaml
metadata:
  materia: "idiomas-extranjeros/aleman"
  tema: "konjunktiv-ii-completo"
  nivel: "B2"
  tags: ["deseos", "präteritum"]
pasos:
  - "Identifica la conjugación de 'werden' en Konjunktiv II."
  - "Completa el hueco con la forma correcta del verbo."
respuestas_validas:
  - "Würde werden"
tipo: completar
enunciado: "Wenn ich ___ alt wäre, würde ich nicht mehr arbeiten."  
explicacion: "'Werden' en Konjunktiv II es 'würde werden', que se usa para expresar deseos hipotéticos. La forma präteritum no se aplica aquí porque el verbo principal ya está en Konjunktiv II (señalado por 'würde')."
```




### 16 — Elección de la opción correcta  
```yaml
metadata:
  materia: "idiomas-extranjeros/aleman"
  tema: "konjunktiv-ii-completo"
  nivel: "B2"
  tags: ["opciones", "hypothetische"]
pasos:
  - "Selecciona la forma correcta de 'singen' en Konjunktiv II."
  - "Asegúrate de que el verbo esté en el tiempo adecuado para una hipótesis."
respuesta: "würde singen"
opciones_explicitas:
  - "sing"
  - "würde singen"
  - "sang"
tipo: mc
enunciado: "Welche Form ist korrekt? 'Er ___ ein Lied.'"  
explicacion: "'Würde singen' es la forma correcta, ya que el Konjunktiv II con 'würde' se usa para expresar acciones hipotéticas en el presente."
```




### 17 — Completar un contexto de sugerencia  
```yaml
metadata:
  materia: "idiomas-extranjeros/aleman"
  tema: "konjunktiv-ii-completo"
  nivel: "B2"
  tags: ["sugerencias", "würde"]
pasos:
  - "Identifica la forma correcta de 'gehen' en Konjunktiv II."
  - "Completa el hueco con la conjugación adecuada del verbo."
respuestas_validas:
  - "Würde gehen"
tipo: completar
enunciado: "Du könntest ___ zum Park, wenn du Zeit hast."  
explicacion: "'Gehen' en Konjunktiv II es 'würde gehen', que se usa para sugerir acciones hipotéticas. La forma präteritum no se aplica aquí porque el verbo principal ya está en Konjunktiv II (señalado por 'könntest')."
```




### 18 — Elección de la opción correcta  
```yaml
metadata:
  materia: "idiomas-extranjeros/aleman"
  tema: "konjunktiv-ii-completo"
  nivel: "B2"
  tags: ["opciones", "hypothetische"]
pasos:
  - "Selecciona la forma correcta de 'schlafen' en Konjunktiv II."
  - "Asegúrate de que el verbo esté en el tiempo adecuado para una hipótesis."
respuesta: "würde schlafen"
opciones_explicitas:
  - "schlafe"
  - "würde schlafen"
  - "schlief"
tipo: mc
enunciado: "Welche Form ist korrekt? 'Ich ___ in der Bibliothek.'"  
explicacion: "'Würde schlafen' es la forma correcta, ya que el Konjunktiv II con 'würde' se usa para expresar acciones hipotéticas en el presente."
```




### 19 — Completar un contexto de deseo  
```yaml
metadata:
  materia: "idiomas-extranjeros/aleman"
  tema: "konjunktiv-ii-completo"
  nivel: "B2"
  tags: ["deseos", "präteritum"]
pasos:
  - "Identifica la conjugación de 'haben' en Konjunktiv II."
  - "Completa el hueco con la forma correcta del verbo."
respuestas_validas:
  - "Hätte"
tipo: completar
enunciado: "Wenn ich ___ mehr Freizeit hätte, würde ich reisen."  
explicacion: "'Haben' en Konjunktiv II es 'hätte', que se usa para expresar deseos hipotéticos. La forma präteritum no se aplica aquí porque el verbo principal ya está en Konjunktiv II (señalado por 'würde')."
```




### 20 — Elección de la opción correcta  
```
metadata:  
  materia: "aleman"  
  tema: "konjunktiv-ii-completo"  
  nivel: "B2"  
  tags: ["hypothetische Situationen", "lassen"]  
pasos:  
  - "Identifica la forma correcta del Konjunktiv II para el verbo 'lassen' en esta oración."  
explicacion: "En oraciones hipotéticas, se usa 'würde + infinitivo' o formas irregulares. El verbo 'lassen' en Konjunktiv II es 'ließe', pero cuando va seguido de un objeto directo, se usa el infinitivo del verbo que sigue. Aquí, la estructura requiere 'lassen' en su forma correcta."  
tipo: mc  
opciones_explicitas:  
  - "Er würde das Fenster öffnen."  
  - "Er ließe das Fenster öffnen."  
  - "Er lässt das Fenster öffnen."  
  - "Er würde das Fenster geöffnet."  
respuesta: "Er ließe das Fenster öffnen."

enunciado: "¿Cuál oración usa correctamente el Konjunktiv II de 'lassen' en un contexto hipotético?"

```

### 21 — Completar un contexto de sugerencia  
```
metadata:  
  materia: "aleman"  
  tema: "konjunktiv-ii-completo"  
  nivel: "B2"  
  tags: ["hypothetische Situationen", "geben"]  
pasos:  
  - "Completa el hueco con la forma correcta del Konjunktiv II para el verbo 'geben'."  
explicacion: "El verbo 'geben' en Konjunktiv II es 'gäbe'. Aquí, se requiere la forma conjugada en primera persona singular (ich) o tercera persona singular (er/sie/es), dependiendo del contexto. La oración implica una situación hipotética."  
tipo: completar  
respuestas_validas:
  - "gäbe"
  - "Gäbe"

enunciado: "Wenn es keine Probleme ___, wäre das Leben einfacher."

```

### 22 — Elección de la opción correcta  
```
metadata:  
  materia: "aleman"  
  tema: "konjunktiv-ii-completo"  
  nivel: "B2"  
  tags: ["condicionales", "haben"]  
pasos:  
  - "Identifica la forma correcta del Konjunktiv II para el verbo 'haben' en esta oración."  
explicacion: "Cuando se expresa una condición hipotética, se usa el Konjunktiv II con 'würde + infinitivo'. Aquí, 'haben' se transforma en 'hätte', pero la estructura requiere la forma adecuada para la persona y número."  
tipo: mc  
opciones_explicitas:  
  - "Wenn ich mehr Zeit hätte, würde ich reisen."  
  - "Wenn ich mehr Zeit haben, würde ich reisen."  
  - "Wenn ich mehr Zeit hätt, würde ich reisen."  
  - "Wenn ich mehr Zeit habe, würde ich reisen."  
respuesta: "Wenn ich mehr Zeit hätte, würde ich reisen."

enunciado: "¿Cuál oración usa correctamente el Konjunktiv II de 'haben' en una oración condicional?"

```

### 23 — Completar un contexto de deseo  
```
metadata:  
  materia: "aleman"  
  tema: "konjunktiv-ii-completo"  
  nivel: "B2"  
  tags: ["objeto indirecto", "kennen"]  
pasos:  
  - "Completa el hueco con la forma correcta del Konjunktiv II para el verbo 'kennen'."  
explicacion: "El verbo 'kennen' en Konjunktiv II es 'kennte'. Aquí, se requiere su conjugación correcta según el sujeto de la oración."
tipo: completar

enunciado: "Wenn ich ihn besser ___, würde ich ihm vertrauen."

respuestas_validas:
  - "kennte"
  - "Kennte"
```
