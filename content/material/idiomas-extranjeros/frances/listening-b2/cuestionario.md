# Idiomas — Francés — listening-b2 (cuestionario)

> Promovido desde el borrador (`../../../_borradores-gemma/idiomas-extranjeros/frances/listening-b2/cuestionario_crudo.md`)
> con una corrección mecánica: el bloque 23 tenía comillas dobles anidadas dentro de varios
> ítems de `pasos:` ya entrecomillados, que cerraban la cadena antes de tiempo; se cambiaron las
> comillas internas a simples.

---

### 1 — Futur proche  
```
metadata:  
  materia: "idiomas-extranjeros/frances"  
  tema: "listening-b2"  
  nivel: "B2"  
  tags: ["futur", "aller"]  
pasos:  
  - "Identificar el uso del futur proche con 'aller'."  
explicacion: "El futuro próximo en francés se forma con 'aller' + infinitivo. La oración indica una acción próxima, por lo que la respuesta correcta es 'vas'."  
enunciado: "Qu'est-ce que tu ___ faire demain ?"  
tipo: mc  
opciones_explicitas:  
  - "vas"  
  - "verras"  
  - "feras"  
respuesta: "vas"  

```



### 2 — Concordancia de participio passé  
```
metadata:  
  materia: "idiomas-extranjeros/frances"  
  tema: "listening-b2"  
  nivel: "B2"  
  tags: ["passé composé", "concordance"]  
pasos:  
  - "Verificar si el participio pasado concuerda con el sujeto."  
explicacion: "El verbo 'envoyer' no requiere concordancia porque está conjugado con 'avoir'. La forma correcta es 'a envoyé'."  
enunciado: "Elle ___ les lettres hier."  
tipo: completar  
respuestas_validas:  
  - "a envoyé"  
  - "A envoyé"  

```



### 3 — Impératif (tu)  
```
metadata:  
  materia: "idiomas-extranjeros/frances"  
  tema: "listening-b2"  
  nivel: "B2"  
  tags: ["impératif", "ordre"]  
pasos:  
  - "Identificar la forma del imperativo para 'tu'."  
explicacion: "El imperativo para 'tu' se forma con la raíz del verbo. La forma correcta de 'regarder' es 'regarde'."  
enunciado: "___ ce tableau, s'il te plaît !"  
tipo: completar  
respuestas_validas:  
  - "Regarde"  
  - "regarde"  

```



### 4 — Futur simple (ils)  
```
metadata:  
  materia: "idiomas-extranjeros/frances"  
  tema: "listening-b2"  
  nivel: "B2"  
  tags: ["futur", "ils"]  
pasos:  
  - "Aplicar la conjugación del futur simple para 'ils'."  
explicacion: "El futuro simple de 'aller' para 'ils' es 'iront'. La oración habla de una acción futura confirmada."  
enunciado: "Ils ___ à Paris l'an prochaine."  
tipo: completar  
respuestas_validas:  
  - "iront"  
  - "Irunt"  

```



### 5 — Participe passé (être)  
```
metadata:  
  materia: "idiomas-extranjeros/frances"  
  tema: "listening-b2"  
  nivel: "B2"  
  tags: ["passé composé", "être"]  
pasos:  
  - "Determinar si el participio pasado concuerda con el sujeto."  
explicacion: "El verbo 'aller' se conjuga con 'être', por lo que el participio 'allé' debe concordar en número y género. La forma correcta es 'sont allés'."  
enunciado: "Les élèves ___ à la sortie de l'école."  
tipo: completar  
respuestas_validas:  
  - "sont allés"  
  - "Sont allés"  

```



### 6 — Conditionnel présent (je)  
```
metadata:  
  materia: "idiomas-extranjeros/frances"  
  tema: "listening-b2"  
  nivel: "B2"  
  tags: ["conditionnel", "hypothèse"]  
pasos:  
  - "Identificar la conjugación del conditionnel presente para 'je'."  
explicacion: "El conditionnel presente de 'parler' se forma con la raíz + 'ais'. La forma correcta es 'parlerais'."  
enunciado: "Si j'avais plus de temps, je ___ avec toi."  
tipo: completar  
respuestas_validas:  
  - "parlerais"  
  - "Parlerais"  

```



### 7 — Impératif (nous)  
```
metadata:  
  materia: "idiomas-extranjeros/frances"  
  tema: "listening-b2"  
  nivel: "B2"  
  tags: ["impératif", "ordre"]  
pasos:  
  - "Seleccionar la forma del imperativo para 'nous'."  
explicacion: "El imperativo para 'nous' se conjuga con la raíz + 'ons'. La forma correcta de 'manger' es ' mangeons'."  
enunciado: "___ ce dessert, c'est délicieux !"  
tipo: completar  
respuestas_validas:  
  - "Mangeons"  
  - "mangeons"  

```



### 8 — Futur proche (ils)  
```
metadata:  
  materia: "idiomas-extranjeros/frances"  
  tema: "listening-b2"  
  nivel: "B2"  
  tags: ["futur", "aller"]  
pasos:  
  - "Aplicar el futuro próximo con 'aller' para 'ils'."  
explicacion: "El futuro próximo de 'aller' para 'ils' es 'iront'. La oración describe una acción próxima."  
enunciado: "Ils ___ à la plage ce week-end."  
tipo: completar  
respuestas_validas:  
  - "iront"  
  - "Irunt"  

```



### 9 — Participe passé (être)  
```
metadata:  
  materia: "idiomas-extranjeros/frances"  
  tema: "listening-b2"  
  nivel: "B2"  
  tags: ["passé composé", "être"]  
pasos:  
  - "Verificar la concordancia del participio pasado con el sujeto."  
explicacion: "El verbo 'venir' se conjuga con 'être', por lo que el participio 'venu' debe concordar en número y género. La forma correcta es 'sont venus'."  
enunciado: "Les invités ___ à la fête hier soir."  
tipo: completar  
respuestas_validas:  
  - "sont venus"  
  - "Sont venus"  

```



### 10 — Conditionnel présent (tu)  
```
metadata:  
  materia: "idiomas-extranjeros/frances"  
  tema: "listening-b2"  
  nivel: "B2"  
  tags: ["conditionnel", "hypothèse"]  
pasos:  
  - "Identificar la conjugación del conditionnel presente para 'tu'."  
explicacion: "El conditionnel presente de 'finir' se forma con la raíz + 'ais'. La forma correcta es 'finirais'."  
enunciado: "Si tu avais plus d'énergie, tu ___ ce projet."  
tipo: completar  
respuestas_validas:  
  - "finirais"  
  - "Finirais"  

```



### 11 — Impératif (il)  
```
metadata:  
  materia: "idiomas-extranjeros/frances"  
  tema: "listening-b2"  
  nivel: "B2"  
  tags: ["impératif", "ordre"]  
pasos:  
  - "Seleccionar la forma del imperativo para 'il'."  
explicacion: "El imperativo para 'il' se conjuga con la raíz + 'e'. La forma correcta de 'regarder' es 'regarde'."  
enunciado: "___ ce message, c'est important !"  
tipo: completar  
respuestas_validas:  
  - "Regarde"  
  - "regarde"  

```



### 12 — Futur simple (tu)  
```
metadata:  
  materia: "idiomas-extranjeros/frances"  
  tema: "listening-b2"  
  nivel: "B2"  
  tags: ["futur", "tu"]  
pasos:  
  - "Aplicar la conjugación del futur simple para 'tu'."  
explicacion: "El futuro simple de 'aller' para 'tu' es 'iras'. La oración describe una acción futura confirmada."  
enunciado: "Tu ___ à Lyon demain, n'est-ce pas ?"  
tipo: completar  
respuestas_validas:  
  - "iras"  
  - "Iras"  

```



### 13 — Participe passé (avoir)  
```
metadata:  
  materia: "idiomas-extranjeros/frances"  
  tema: "listening-b2"  
  nivel: "B2"  
  tags: ["passé composé", "avoir"]  
pasos:  
  - "Verificar si el participio pasado concuerda con el sujeto."  
explicacion: "El verbo 'parler' se conjuga con 'avoir', por lo que no requiere concordancia. La forma correcta es 'a parlé'."  
enunciado: "Il ___ avec ses amis hier soir."  
tipo: completar  
respuestas_validas:  
  - "a parlé"  
  - "A parlé"  

```



### 14 — Conditionnel présent (nous)  
```
metadata:  
  materia: "idiomas-extranjeros/frances"  
  tema: "listening-b2"  
  nivel: "B2"  
  tags: ["conditionnel", "hypothèse"]  
pasos:  
  - "Identificar la conjugación del conditionnel presente para 'nous'."  
explicacion: "El conditionnel presente de 'aller' se forma con la raíz + 'ions'. La forma correcta es 'irions'."  
enunciado: "Si nous avions plus d'argent, nous ___ voyager en Asie."  
tipo: completar  
respuestas_validas:  
  - "irions"  
  - "Irions"  

```



### 15 — Impératif (vous)  
```
metadata:  
  materia: "idiomas-extranjeros/frances"  
  tema: "listening-b2"  
  nivel: "B2"  
  tags: ["impératif", "ordre"]  
pasos:  
  - "Seleccionar la forma del imperativo para 'vous'."  
explicacion: "El imperativo para 'vous' se conjuga con la raíz + 'ez'. La forma correcta de 'manger' es ' mangez'."  
enunciado: "___ ce repas, c'est prêt !"  
tipo: completar  
respuestas_validas:  
  - "Mangez"  
  - "mangez"  

```



### 16 — Futur proche (elle)  
```
metadata:  
  materia: "idiomas-extranjeros/frances"  
  tema: "listening-b2"  
  nivel: "B2"  
  tags: ["futur", "aller"]  
pasos:  
  - "Aplicar el futuro próximo con 'aller' para 'elle'."  
explicacion: "El futuro próximo de 'aller' para 'elle' es 'ira'. La oración describe una acción próxima."  
enunciado: "Elle ___ au cinéma ce soir, n'est-ce pas ?"  
tipo: completar  
respuestas_validas:  
  - "ira"  
  - "Ira"  

```



### 17 — Participe passé (être)  
```
metadata:  
  materia: "idiomas-extranjeros/frances"  
  tema: "listening-b2"  
  nivel: "B2"  
  tags: ["passé composé", "être"]  
pasos:  
  - "Verificar la concordancia del participio pasado con el sujeto."  
explicacion: "El verbo 'naître' se conjuga con 'être', por lo que el participio 'né' debe concordar en género. La forma correcta es 'est né'."  
enunciado: "Mon frère ___ le 15 mars."  
tipo: completar  
respuestas_validas:  
  - "est né"  
  - "Est né"  

```



### 18 — Conditionnel présent (ils)  
```
metadata:  
  materia: "idiomas-extranjeros/frances"  
  tema: "listening-b2"  
  nivel: "B2"  
  tags: ["conditionnel", "hypothèse"]  
pasos:  
  - "Identificar la conjugación del conditionnel presente para 'ils'."  
explicacion: "El conditionnel presente de 'finir' se forma con la raíz + 'aient'. La forma correcta es 'finiraient'."  
enunciado: "Si nous avions plus de temps, ils ___ ce travail."  
tipo: completar  
respuestas_validas:  
  - "finiraient"  
  - "Finiraient"  

```



### 19 — Impératif (je)  
```
metadata:  
  materia: "idiomas-extranjeros/frances"  
  tema: "listening-b2"  
  nivel: "B2"  
  tags: ["impératif", "ordre"]  
pasos:  
  - "Seleccionar la forma del imperativo para 'je'."  
explicacion: "El imperativo para 'je' se conjuga con la raíz + 'e'. La forma correcta de 'manger' es 'mange'."  
enunciado: "___ ce fruit, c'est bon pour la santé !"  
tipo: completar  
respuestas_validas:  
  - "Mange"  
  - "mange"  

```



### 20 — Futur simple (elle)  
```
metadata:  
  materia: "idiomas-extranjeros/frances"  
  tema: "listening-b2"  
  nivel: "B2"  
  tags: ["futur", "elle"]  
pasos:  
  - "Aplicar la conjugación del futur simple para 'elle'."  
explicacion: "El futuro simple de 'aller' para 'elle' es 'ira'. La oración describe una acción futura confirmada."  
enunciado: "Elle ___ à l'école demain, n'est-ce pas ?"  
tipo: completar  
respuestas_validas:  
  - "ira"  
  - "Ira"  

```



### 21 — Participe passé (avoir)  
```
metadata:  
  materia: "idiomas-extranjeros/frances"  
  tema: "listening-b2"  
  nivel: "B2"  
  tags: ["passé composé", "avoir"]  
pasos:  
  - "Verificar si el participio pasado concuerda con el sujeto."  
explicacion: "El verbo 'parler' se conjuga con 'avoir', por lo que no requiere concordancia. La forma correcta es 'a parlé'."  
enunciado: "Nous ___ avec nos parents hier soir."  
tipo: completar  
respuestas_validas:  
  - "avons parlé"  
  - "Avons parlé"  

```



### 22 — Conditionnel présent (vous)  
```
metadata:  
  materia: "idiomas-extranjeros/frances"  
  tema: "listening-b2"  
  nivel: "B2"  
  tags: ["conditionnel", "hypothèse"]  
pasos:  
  - "Identificar la conjugación del conditionnel presente para 'vous'."  
explicacion: "El conditionnel presente de 'aller' se forma con la raíz + 'iez'. La forma correcta es 'iriez'."  
enunciado: "Si vous aviez plus d'énergie, vous ___ ce voyage."  
tipo: completar  
respuestas_validas:  
  - "iriez"  
  - "Iriez"  

```



### 23 — Impératif (ils)  
```  
metadata:  
  materia: "frances"  
  tema: "listening-b2"  
  nivel: "B2"  
  tags: ["futur", "aller"]  
pasos:  
  - "Identificar el sujeto ('nous') y el tiempo verbal (futuro próximo)."
  - "Recordar la estructura del futuro próximo: 'aller + infinitivo'."
  - "Seleccionar la forma conjugada de 'aller' para 'nous'."
respuestas_validas:  
  - "allons"  
  - "Allons"  
tipo: completar  
enunciado: "Nous ___ en vacances la semaine prochaine."  
explicacion: "La conjunción 'nous' con el futuro próximo requiere 'allons'. El verbo 'aller' en futuro es 'iré', pero en el futuro próximo se usa 'aller + infinitivo', por lo que la forma correcta es 'allons'."  
```

### 24 — Futur proche (nous)  
```
metadata:  
  materia: "idiomas-extranjeros/frances"  
  tema: "listening-b2"  
  nivel: "B2"  
  tags: ["futur", "aller"]  
pasos:  
  - "Aplicar el futuro próximo con 'aller' para 'nous'."  
explicacion: "El futuro próximo de 'aller' para 'nous' es 'allons'. La oración describe una acción próxima."  
enunciado: "Nous ___ au restaurant ce soir, n'est-ce pas ?"  
tipo: completar  
respuestas_validas:  
  - "allons"  
  - "Allons"  

```



### 25 — Participe passé (être)  
```
metadata:  
  materia: "idiomas-extranjeros/frances"  
  tema: "listening-b2"  
  nivel: "B2"  
  tags: ["passé composé", "être"]  
pasos:  
  - "Verificar la concordancia del participio pasado con el sujeto."  
explicacion: "El verbo 'mourir' se conjuga con 'être', por lo que el participio 'mort' debe concordar en género. La forma correcta es 'est morte'."  
enunciado: "Ma sœur ___ hier dans un accident."  
tipo: completar  
respuestas_validas:  
  - "est morte"  
  - "Est morte"
```
