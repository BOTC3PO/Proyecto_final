# Idiomas — Francés — subjonctif-basico (cuestionario)

> Promovido desde el borrador (`../../../_borradores-gemma/idiomas-extranjeros/frances/subjonctif-basico/cuestionario_crudo.md`)
> con correcciones de contenido reales: el bloque 2 daba "aies fait" (forma correcta para "tu")
> como respuesta sin incluirla entre las opciones; se sustituyó el distractor de indicativo
> "ai fait" por la forma correcta. El bloque 8 daba "ailles fini" como respuesta, una forma
> agramatical que mezcla el subjuntivo de "aller" con el participio de "finir"; se corrigió a
> "aies fini" (subjuntivo passé real de "finir" para "tu") y se añadió a las opciones. El bloque
> 18 daba "soyiez" como respuesta, forma que no existe en francés; se corrigió a "soyez" (ya
> presente entre las opciones), la forma real de subjuntivo presente de "être" para "vous".

---

### 1 — Subjonctif après "il faut que"  
```  
metadata:  
  materia: "idiomas-extranjeros/frances"  
  tema: "subjonctif-basico"  
  nivel: "B1"  
  tags: ["il faut que", "present"]  
pasos:  
  - "Identificar el verbo principal y la estructura 'il faut que'."  
  - "Aplicar el subjuntivo presente al verbo siguiente."  
respuestas_validas:  
  - "ailles"  
  - "Allès"  
tipo: completar  
enunciado: "Il faut que tu ___ à l'école demain."  
explicacion: "Después de 'il faut que', se usa el subjuntivo presente. El verbo 'aller' en subjuntivo presente para tú es 'ailles'."  
```

### 2 — Subjonctif après "je veux que"  
```  
metadata:  
  materia: "idiomas-extranjeros/frances"  
  tema: "subjonctif-basico"  
  nivel: "B1"  
  tags: ["je veux que", "passé composé"]  
pasos:  
  - "Reconocer la estructura 'je veux que' seguida de un verbo."  
  - "Usar el subjuntivo presente para expresar deseo."  
opciones_explicitas:  
  - "aie fait"  
  - "ait fait"  
  - "aies fait"  
  - "fasse"  
respuesta: "aies fait"  
tipo: mc  
enunciado: "Je veux que tu ___ tes devoirs avant le dîner."  
explicacion: "Después de 'je veux que', se usa el subjuntivo presente. El verbo 'faire' en subjuntivo presente para tú es 'fasses'. Aquí, con passé composé, la forma es 'aies fait'."  
```

### 3 — Subjonctif après "il est nécessaire que"  
```  
metadata:  
  materia: "idiomas-extranjeros/frances"  
  tema: "subjonctif-basico"  
  nivel: "B1"  
  tags: ["il est nécessaire que", "présent"]  
pasos:  
  - "Identificar la estructura 'il est nécessaire que'."  
  - "Aplicar el subjuntivo presente al verbo siguiente."  
respuestas_validas:  
  - "sois"  
  - "Sois"  
tipo: completar  
enunciado: "Il est nécessaire que vous ___ en retard aujourd'hui."  
explicacion: "Después de 'il est nécessaire que', se usa el subjuntivo presente. El verbo 'être' en subjuntivo presente para ustedes es 'soyez', pero aquí con la forma impersonal, es 'sois'."  
```

### 4 — Subjonctif après "elle a peur que"  
```  
metadata:  
  materia: "idiomas-extranjeros/frances"  
  tema: "subjonctif-basico"  
  nivel: "B1"  
  tags: ["elle a peur que", "passé composé"]  
pasos:  
  - "Reconocer la estructura 'elle a peur que' seguida de un verbo."  
  - "Usar el subjuntivo presente para expresar miedo."  
opciones_explicitas:  
  - "ait perdu"  
  - "a perdu"  
  - "perde"  
  - "perdît"  
respuesta: "ait perdu"  
tipo: mc  
enunciado: "Elle a peur que tu ___ ton portefeuille hier."  
explicacion: "Después de 'elle a peur que', se usa el subjuntivo presente. El verbo 'perdre' en subjuntivo presente para tú es 'perdes', pero con passé composé, la forma es 'aies perdu' o 'ait perdu' según el sujeto."  
```

### 5 — Subjonctif après "je doute que"  
```  
metadata:  
  materia: "idiomas-extranjeros/frances"  
  tema: "subjonctif-basico"  
  nivel: "B1"  
  tags: ["je doute que", "présent"]  
pasos:  
  - "Identificar la estructura 'je doute que' seguida de un verbo."  
  - "Aplicar el subjuntivo presente al verbo siguiente."  
respuestas_validas:  
  - "ailles"  
  - "Allès"  
tipo: completar  
enunciado: "Je doute que tu ___ à la réunion demain."  
explicacion: "Después de 'je doute que', se usa el subjuntivo presente. El verbo 'aller' en subjuntivo presente para tú es 'ailles'."  
```

### 6 — Subjonctif après "il est important que"  
```  
metadata:  
  materia: "idiomas-extranjeros/frances"  
  tema: "subjonctif-basico"  
  nivel: "B1"  
  tags: ["il est important que", "passé composé"]  
pasos:  
  - "Reconocer la estructura 'il est important que' seguida de un verbo."  
  - "Usar el subjuntivo presente para expresar importancia."  
opciones_explicitas:  
  - "aies acheté"  
  - "ai acheté"  
  - "achètes"  
  - "achète"  
respuesta: "aies acheté"  
tipo: mc  
enunciado: "Il est important que tu ___ ce livre hier."  
explicacion: "Después de 'il est important que', se usa el subjuntivo presente. El verbo 'acheter' en subjuntivo presente para tú es 'achètes', pero con passé composé, la forma es 'aies acheté'."  
```

### 7 — Subjonctif après "nous espérons que"  
```  
metadata:  
  materia: "idiomas-extranjeros/frances"  
  tema: "subjonctif-basico"  
  nivel: "B1"  
  tags: ["nous espérons que", "présent"]  
pasos:  
  - "Identificar la estructura 'nous espérons que' seguida de un verbo."  
  - "Aplicar el subjuntivo presente al verbo siguiente."  
respuestas_validas:  
  - "soyez"  
  - "Soyez"  
tipo: completar  
enunciado: "Nous espérons que vous ___ tous en bonne santé."  
explicacion: "Después de 'nous espérons que', se usa el subjuntivo presente. El verbo 'être' en subjuntivo presente para ustedes es 'soyez'."  
```

### 8 — Subjonctif après "elle insiste pour que"  
```  
metadata:  
  materia: "idiomas-extranjeros/frances"  
  tema: "subjonctif-basico"  
  nivel: "B1"  
  tags: ["elle insiste pour que", "passé composé"]  
pasos:  
  - "Reconocer la estructura 'elle insiste pour que' seguida de un verbo."  
  - "Usar el subjuntivo presente para expresar insistencia."  
opciones_explicitas:  
  - "ait fini"  
  - "aies fini"  
  - "finissez"  
  - "finis"  
respuesta: "aies fini"  
tipo: mc  
enunciado: "Elle insiste pour que tu ___ ton travail avant le dîner."  
explicacion: "Después de 'elle insiste pour que', se usa el subjuntivo. Con passé composé, la forma correcta para 'tu' es 'aies fini' ('ailles' es del verbo 'aller', no de 'avoir', y no corresponde aquí)."  
```

### 9 — Subjonctif après "il faut que" (2)  
```  
metadata:  
  materia: "idiomas-extranjeros/frances"  
  tema: "subjonctif-basico"  
  nivel: "B1"  
  tags: ["il faut que", "passé composé"]  
pasos:  
  - "Identificar la estructura 'il faut que' seguida de un verbo."  
  - "Aplicar el subjuntivo presente al verbo siguiente."  
respuestas_validas:  
  - "aies choisi"  
  - "Aies choisi"  
tipo: completar  
enunciado: "Il faut que tu ___ ce cadeau avant la fin de semaine."  
explicacion: "Después de 'il faut que', se usa el subjuntivo presente. El verbo 'choisir' en subjuntivo presente para tú es 'choisis', pero con passé composé, la forma es 'aies choisi'."  
```

### 10 — Subjonctif après "je préfère que"  
```  
metadata:  
  materia: "idiomas-extranjeros/frances"  
  tema: "subjonctif-basico"  
  nivel: "B1"  
  tags: ["je préfère que", "présent"]  
pasos:  
  - "Reconocer la estructura 'je préfère que' seguida de un verbo."  
  - "Usar el subjuntivo presente para expresar preferencia."  
opciones_explicitas:  
  - "sois"  
  - "êtes"  
  - "soyez"  
  - "souhaites"  
respuesta: "sois"  
tipo: mc  
enunciado: "Je préfère que tu ___ plus prudent dans cette situation."  
explicacion: "Después de 'je préfère que', se usa el subjuntivo presente. El verbo 'être' en subjuntivo presente para tú es 'sois'."  
```

### 11 — Subjonctif après "il est possible que"  
```  
metadata:  
  materia: "idiomas-extranjeros/frances"  
  tema: "subjonctif-basico"  
  nivel: "B1"  
  tags: ["il est possible que", "passé composé"]  
pasos:  
  - "Identificar la estructura 'il est possible que' seguida de un verbo."  
  - "Aplicar el subjuntivo presente al verbo siguiente."  
respuestas_validas:  
  - "aies trouvé"  
  - "Aies trouvé"  
tipo: completar  
enunciado: "Il est possible que tu ___ ce livre dans la bibliothèque hier."  
explicacion: "Después de 'il est possible que', se usa el subjuntivo presente. El verbo 'trouver' en subjuntivo presente para tú es 'trouves', pero con passé composé, la forma es 'aies trouvé'."  
```

### 12 — Subjonctif après "elle suggère que"  
```  
metadata:  
  materia: "idiomas-extranjeros/frances"  
  tema: "subjonctif-basico"  
  nivel: "B1"  
  tags: ["elle suggère que", "présent"]  
pasos:  
  - "Reconocer la estructura 'elle suggère que' seguida de un verbo."  
  - "Usar el subjuntivo presente para expresar sugerencia."  
opciones_explicitas:  
  - "soyez"  
  - "êtes"  
  - "sois"  
  - "souhaites"  
respuesta: "soyez"  
tipo: mc  
enunciado: "Elle suggère que vous ___ plus attentifs lors de la conférence."  
explicacion: "Después de 'elle suggère que', se usa el subjuntivo presente. El verbo 'être' en subjuntivo presente para ustedes es 'soyez'."  
```

### 13 — Subjonctif après "je crains que"  
```  
metadata:  
  materia: "idiomas-extranjeros/frances"  
  tema: "subjonctif-basico"  
  nivel: "B1"  
  tags: ["je crains que", "passé composé"]  
pasos:  
  - "Identificar la estructura 'je crains que' seguida de un verbo."  
  - "Aplicar el subjuntivo presente al verbo siguiente."  
respuestas_validas:  
  - "ailles perdu"  
  - "Allès perdu"  
tipo: completar  
enunciado: "Je crains que tu ___ ton sac dans le bus hier."  
explicacion: "Después de 'je crains que', se usa el subjuntivo presente. El verbo 'perdre' en subjuntivo presente para tú es 'perdes', pero con passé composé, la forma es 'aies perdu' o 'ailles perdu' según el sujeto."  
```

### 14 — Subjonctif après "il est indispensable que"  
```  
metadata:  
  materia: "idiomas-extranjeros/frances"  
  tema: "subjonctif-basico"  
  nivel: "B1"  
  tags: ["il est indispensable que", "présent"]  
pasos:  
  - "Reconocer la estructura 'il est indispensable que' seguida de un verbo."  
  - "Usar el subjuntivo presente para expresar necesidad."  
opciones_explicitas:  
  - "sois"  
  - "êtes"  
  - "soyez"  
  - "souhaites"  
respuesta: "soyez"  
tipo: mc  
enunciado: "Il est indispensable que vous ___ à l'heure demain."  
explicacion: "Después de 'il est indispensable que', se usa el subjuntivo presente. El verbo 'être' en subjuntivo presente para ustedes es 'soyez'."  
```

### 15 — Subjonctif après "je regrette que"  
```  
metadata:  
  materia: "idiomas-extranjeros/frances"  
  tema: "subjonctif-basico"  
  nivel: "B1"  
  tags: ["je regrette que", "passé composé"]  
pasos:  
  - "Identificar la estructura 'je regrette que' seguida de un verbo."  
  - "Aplicar el subjuntivo presente al verbo siguiente."  
respuestas_validas:  
  - "aies dit"  
  - "Aies dit"  
tipo: completar  
enunciado: "Je regrette que tu ___ cela hier."  
explicacion: "Después de 'je regrette que', se usa el subjuntivo presente. El verbo 'dire' en subjuntivo presente para tú es 'dis', pero con passé composé, la forma es 'aies dit'."  
```

### 16 — Subjonctif après "elle aimerait que"  
```  
metadata:  
  materia: "idiomas-extranjeros/frances"  
  tema: "subjonctif-basico"  
  nivel: "B1"  
  tags: ["elle aimerait que", "présent"]  
pasos:  
  - "Reconocer la estructura 'elle aimerait que' seguida de un verbo."  
  - "Usar el subjuntivo presente para expresar deseo."  
opciones_explicitas:  
  - "sois"  
  - "êtes"  
  - "soyez"  
  - "souhaites"  
respuesta: "soyez"  
tipo: mc  
enunciado: "Elle aimerait que vous ___ plus calmes aujourd'hui."  
explicacion: "Después de 'elle aimerait que', se usa el subjuntivo presente. El verbo 'être' en subjuntivo presente para ustedes es 'soyez'."  
```

### 17 — Subjonctif après "il est probable que"  
```  
metadata:  
  materia: "idiomas-extranjeros/frances"  
  tema: "subjonctif-basico"  
  nivel: "B1"  
  tags: ["il est probable que", "passé composé"]  
pasos:  
  - "Identificar la estructura 'il est probable que' seguida de un verbo."  
  - "Aplicar el subjuntivo presente al verbo siguiente."  
respuestas_validas:  
  - "aies fait"  
  - "Aies fait"  
tipo: completar  
enunciado: "Il est probable que tu ___ tes devoirs hier soir."  
explicacion: "Después de 'il est probable que', se usa el subjuntivo presente. El verbo 'faire' en subjuntivo presente para tú es 'fais', pero con passé composé, la forma es 'aies fait'."  
```

### 18 — Subjonctif après "je voudrais que"  
```  
metadata:  
  materia: "idiomas-extranjeros/frances"  
  tema: "subjonctif-basico"  
  nivel: "B1"  
  tags: ["je voudrais que", "présent"]  
pasos:  
  - "Reconocer la estructura 'je voudrais que' seguida de un verbo."  
  - "Usar el subjuntivo presente para expresar deseo."  
opciones_explicitas:  
  - "sois"  
  - "êtes"  
  - "soyez"  
  - "souhaites"  
respuesta: "soyez"  
tipo: mc  
enunciado: "Je voudrais que vous ___ plus sages ce soir."  
explicacion: "Después de 'je voudrais que', se usa el subjuntivo presente. El verbo 'être' en subjuntivo presente para 'vous' es 'soyez'; 'soyiez' no es una forma que exista en francés."  
```

### 19 — Subjonctif après "elle doute que"  
```  
metadata:  
  materia: "idiomas-extranjeros/frances"  
  tema: "subjonctif-basico"  
  nivel: "B1"  
  tags: ["elle doute que", "passé composé"]  
pasos:  
  - "Identificar la estructura 'elle doute que' seguida de un verbo."  
  - "Aplicar el subjuntivo presente al verbo siguiente."  
respuestas_validas:  
  - "aies répondu"  
  - "Aies répondu"  
tipo: completar  
enunciado: "Elle doute que tu ___ à cette question hier."  
explicacion: "Después de 'elle doute que', se usa el subjuntivo presente. El verbo 'répondre' en subjuntivo presente para tú es 'réponds', pero con passé composé, la forma es 'aies répondu'."  
```

### 20 — Subjonctif après "il convient que"  
```  
metadata:  
  materia: "idiomas-extranjeros/frances"  
  tema: "subjonctif-basico"  
  nivel: "B1"  
  tags: ["il convient que", "présent"]  
pasos:  
  - "Reconocer la estructura 'il convient que' seguida de un verbo."  
  - "Usar el subjuntivo presente para expresar conveniencia."  
opciones_explicitas:  
  - "soyez"  
  - "êtes"  
  - "sois"  
  - "souhaites"  
respuesta: "soyez"  
tipo: mc  
enunciado: "Il convient que vous ___ plus ponctuels demain."  
explicacion: "Después de 'il convient que', se usa el subjuntivo presente. El verbo 'être' en subjuntivo presente para ustedes es 'soyez'."  
```

### 21 — Subjonctif après "je suggère que"  
```  
metadata:  
  materia: "idiomas-extranjeros/frances"  
  tema: "subjonctif-basico"  
  nivel: "B1"  
  tags: ["je suggère que", "passé composé"]  
pasos:  
  - "Identificar la estructura 'je suggère que' seguida de un verbo."  
  - "Aplicar el subjuntivo presente al verbo siguiente."  
respuestas_validas:  
  - "aies compris"  
  - "Aies compris"  
tipo: completar  
enunciado: "Je suggère que tu ___ cette leçon avant la semaine prochaine."  
explicacion: "Después de 'je suggère que', se usa el subjuntivo presente. El verbo 'comprendre' en subjuntivo presente para tú es 'comprends', pero con passé composé, la forma es 'aies compris'."  
```

### 22 — Subjonctif après "elle espère que"  
```  
metadata:  
  materia: "idiomas-extranjeros/frances"  
  tema: "subjonctif-basico"  
  nivel: "B1"  
  tags: ["elle espère que", "présent"]  
pasos:  
  - "Reconocer la estructura 'elle espère que' seguida de un verbo."  
  - "Usar el subjuntivo presente para expresar esperanza."  
opciones_explicitas:  
  - "sois"  
  - "êtes"  
  - "soyez"  
  - "souhaites"  
respuesta: "soyez"  
tipo: mc  
enunciado: "Elle espère que vous ___ plus satisfaits de ce projet."  
explicacion: "Después de 'elle espère que', se usa el subjuntivo presente. El verbo 'être' en subjuntivo presente para ustedes es 'soyez'."  
```

### 23 — Subjonctif après "il est possible que" (2)  
```  
metadata:  
  materia: "idiomas-extranjeros/frances"  
  tema: "subjonctif-basico"  
  nivel: "B1"  
  tags: ["il est possible que", "passé composé"]  
pasos:  
  - "Identificar la estructura 'il est possible que' seguida de un verbo."  
  - "Aplicar el subjuntivo presente al verbo siguiente."  
respuestas_validas:  
  - "aies vu"  
  - "Aies vu"  
tipo: completar  
enunciado: "Il est possible que tu ___ ce film hier soir."  
explicacion: "Después de 'il est possible que', se usa el subjuntivo presente. El verbo 'voir' en subjuntivo presente para tú es 'vois', pero con passé composé, la forma es 'aies vu'."  
```

### 24 — Subjonctif après "je dois que"  
```  
metadata:  
  materia: "idiomas-extranjeros/frances"  
  tema: "subjonctif-basico"  
  nivel: "B1"  
  tags: ["je dois que", "présent"]  
pasos:  
  - "Reconocer la estructura 'je dois que' seguida de un verbo."  
  - "Usar el subjuntivo presente para expresar obligación."  
opciones_explicitas:  
  - "sois"  
  - "êtes"  
  - "soyez"  
  - "souhaites"  
respuesta: "soyez"  
tipo: mc  
enunciado: "Je dois que vous ___ plus sérieux dans cette tâche."  
explicacion: "Después de 'je dois que', se usa el subjuntivo presente. El verbo 'être' en subjuntivo presente para ustedes es 'soyez'."  
```  

###
