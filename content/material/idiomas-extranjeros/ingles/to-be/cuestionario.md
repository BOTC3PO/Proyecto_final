# Inglés — to be (cuestionario, 25 preguntas VBLang)

> Corregido a partir del borrador (`../../../_borradores-gemma/idiomas-extranjeros/ingles/to-be/cuestionario_crudo.md`):
> bugs de sintaxis (punto fuera de comillas, comillas dobles anidadas
> sin escapar) y 4 bloques que etiquetaban como "negativo"/"interrogativo"
> una oración afirmativa (contradicción entre el enunciado real y su
> propia explicación) — recorregidos para que ambos coincidan.

---

### 1 — Subject-Verb Agreement (Affirmative)

```
metadata:
  materia: "ingles"
  tema: "to-be"
  nivel: "A1"
  tags: ["afirmativo", "am"]

pasos:
  - "Identifica el sujeto de la oración."
  - "Aplica la forma correcta del verbo 'to be' en presente para el sujeto dado."

respuestas_validas:
  - "am"
  - "Am"

tipo: completar

enunciado: "I ___ a student. (Fill in the blank with the correct form of 'to be'.)"

explicacion: "El sujeto 'I' requiere la forma 'am' del verbo 'to be' en presente."
```

### 2 — Negative Form with 'He'

```
metadata:
  materia: "ingles"
  tema: "to-be"
  nivel: "A1"
  tags: ["negativo", "is"]

pasos:
  - "Reconoce la forma negativa del verbo 'to be'."
  - "Aplica el sujeto 'he' a la estructura correcta."

respuestas_validas:
  - "is not"
  - "isn't"

tipo: completar

enunciado: "He ___ tired. (Complete the negative form of 'to be'.)"

explicacion: "La forma negativa de 'he is' es 'is not' o su contracción 'isn't'."
```

### 3 — Interrogative with 'Are You'

```
metadata:
  materia: "ingles"
  tema: "to-be"
  nivel: "A1"
  tags: ["interrogativa", "are"]

pasos:
  - "Reconoce la estructura de pregunta con el sujeto 'you'."
  - "Aplica la forma correcta del verbo 'to be' en presente."

respuestas_validas:
  - "Are you"
  - "are you"

tipo: completar

enunciado: "___ hungry? (Complete the question with the correct form of 'to be'.)"

explicacion: "Para preguntar sobre el estado de 'you', se usa 'Are you'."
```

### 4 — Contraction in Affirmative ('She's')

```
metadata:
  materia: "ingles"
  tema: "to-be"
  nivel: "A1"
  tags: ["contracción", "is"]

pasos:
  - "Identifica la contracción de 'she is'."
  - "Aplica al sujeto 'she' correctamente."

respuestas_validas:
  - "'s"

tipo: completar

enunciado: "She ___ a teacher. (Complete with the contraction of 'is'.)"

explicacion: "La contracción de 'she is' es 'she's', que en este hueco se escribe simplemente 's (sin espacio antes)."
```

### 5 — Plural Subject in Affirmative ('They Are')

```
metadata:
  materia: "ingles"
  tema: "to-be"
  nivel: "A1"
  tags: ["afirmativo", "are"]

pasos:
  - "Reconoce un sujeto plural."
  - "Aplica la forma correcta del verbo 'to be' en presente."

respuestas_validas:
  - "are"
  - "Are"

tipo: completar

enunciado: "They ___ my friends. (Complete with the correct form of 'to be'.)"

explicacion: "Los sujetos plurales como 'they' requieren la forma 'are' del verbo 'to be'."
```

### 6 — Interrogative with 'Is It'

```
metadata:
  materia: "ingles"
  tema: "to-be"
  nivel: "A1"
  tags: ["interrogativa", "is"]

pasos:
  - "Reconoce la estructura de pregunta con el sujeto 'it'."
  - "Aplica la forma correcta del verbo 'to be' en presente."

respuestas_validas:
  - "Is it"
  - "is it"

tipo: completar

enunciado: "___ a good idea? (Complete the question with the correct form of 'to be'.)"

explicacion: "Para preguntar sobre el estado de 'it', se usa 'Is it'."
```

### 7 — Negative Form with 'We'

```
metadata:
  materia: "ingles"
  tema: "to-be"
  nivel: "A1"
  tags: ["negativo", "are"]

pasos:
  - "Reconoce la forma negativa del verbo 'to be' para sujeto plural."
  - "Aplica al sujeto 'we' correctamente."

respuestas_validas:
  - "are not"
  - "aren't"

tipo: completar

enunciado: "We ___ ready. (Complete the negative form of 'to be'.)"

explicacion: "La forma negativa de 'we are' es 'are not' o su contracción 'aren't'."
```

### 8 — Contraction in Affirmative ('I'm')

```
metadata:
  materia: "ingles"
  tema: "to-be"
  nivel: "A1"
  tags: ["contracción", "am"]

pasos:
  - "Identifica la contracción de 'I am'."
  - "Aplica al sujeto 'I' correctamente."

respuestas_validas:
  - "'m"

tipo: completar

enunciado: "I ___ happy today. (Complete with the contraction of 'am'.)"

explicacion: "La contracción de 'I am' es 'I'm', que en este hueco se escribe simplemente 'm (sin espacio antes)."
```

### 9 — Interrogative with 'He Is'

```
metadata:
  materia: "ingles"
  tema: "to-be"
  nivel: "A1"
  tags: ["interrogativa", "is"]

pasos:
  - "Reconoce la estructura de pregunta con el sujeto 'he'."
  - "Aplica la forma correcta del verbo 'to be' en presente."

respuestas_validas:
  - "Is he"
  - "is he"

tipo: completar

enunciado: "___ your brother? (Complete the question with the correct form of 'to be'.)"

explicacion: "Para preguntar sobre el estado de 'he', se usa 'Is he'."
```

### 10 — Negative Form with 'They'

```
metadata:
  materia: "ingles"
  tema: "to-be"
  nivel: "A1"
  tags: ["negativo", "are"]

pasos:
  - "Reconoce la forma negativa del verbo 'to be' para sujeto plural."
  - "Aplica al sujeto 'they' correctamente."

respuestas_validas:
  - "are not"
  - "aren't"

tipo: completar

enunciado: "They ___ late. (Complete the negative form of 'to be'.)"

explicacion: "La forma negativa de 'they are' es 'are not' o su contracción 'aren't'."
```

### 11 — Affirmative with 'She Is'

```
metadata:
  materia: "ingles"
  tema: "to-be"
  nivel: "A1"
  tags: ["afirmativo", "is"]

pasos:
  - "Reconoce un sujeto singular."
  - "Aplica la forma correcta del verbo 'to be' en presente."

respuestas_validas:
  - "is"
  - "Is"

tipo: completar

enunciado: "She ___ my sister. (Complete with the correct form of 'to be'.)"

explicacion: "Los sujetos singulares como 'she' requieren la forma 'is' del verbo 'to be'."
```

### 12 — Contraction in Affirmative ('You're')

```
metadata:
  materia: "ingles"
  tema: "to-be"
  nivel: "A1"
  tags: ["contracción", "are"]

pasos:
  - "Identifica la contracción de 'you are'."
  - "Aplica al sujeto 'you' correctamente."

respuestas_validas:
  - "'re"

tipo: completar

enunciado: "You ___ going to the party? (Complete with the contraction of 'are'.)"

explicacion: "La contracción de 'you are' es 'you're', que en este hueco se escribe simplemente 're (sin espacio antes)."
```

### 13 — Interrogative with 'Are We'

```
metadata:
  materia: "ingles"
  tema: "to-be"
  nivel: "A1"
  tags: ["interrogativa", "are"]

pasos:
  - "Reconoce la estructura de pregunta con el sujeto 'we'."
  - "Aplica la forma correcta del verbo 'to be' en presente."

respuestas_validas:
  - "Are we"
  - "are we"

tipo: completar

enunciado: "___ ready for the test? (Complete the question with the correct form of 'to be'.)"

explicacion: "Para preguntar sobre el estado de 'we', se usa 'Are we'."
```

### 14 — Negative Form with 'It'

```
metadata:
  materia: "ingles"
  tema: "to-be"
  nivel: "A1"
  tags: ["negativo", "is"]

pasos:
  - "Reconoce la forma negativa del verbo 'to be' para sujeto singular."
  - "Aplica al sujeto 'it' correctamente."

respuestas_validas:
  - "is not"
  - "isn't"

tipo: completar

enunciado: "It ___ raining. (Complete the negative form of 'to be'.)"

explicacion: "La forma negativa de 'it is' es 'is not' o su contracción 'isn't'."
```

### 15 — Contraction in Affirmative ('He's')

```
metadata:
  materia: "ingles"
  tema: "to-be"
  nivel: "A1"
  tags: ["contracción", "is"]

pasos:
  - "Identifica la contracción de 'he is'."
  - "Aplica al sujeto 'he' correctamente."

respuestas_validas:
  - "'s"

tipo: completar

enunciado: "He ___ a doctor. (Complete with the contraction of 'is'.)"

explicacion: "La contracción de 'he is' es 'he's', que en este hueco se escribe simplemente 's (sin espacio antes)."
```

### 16 — Interrogative with 'Is She'

```
metadata:
  materia: "ingles"
  tema: "to-be"
  nivel: "A1"
  tags: ["interrogativa", "is"]

pasos:
  - "Reconoce la estructura de pregunta con el sujeto 'she'."
  - "Aplica la forma correcta del verbo 'to be' en presente."

respuestas_validas:
  - "Is she"
  - "is she"

tipo: completar

enunciado: "___ your teacher? (Complete the question with the correct form of 'to be'.)"

explicacion: "Para preguntar sobre el estado de 'she', se usa 'Is she'."
```

### 17 — Negative Form with 'You'

```
metadata:
  materia: "ingles"
  tema: "to-be"
  nivel: "A1"
  tags: ["negativo", "are"]

pasos:
  - "Reconoce la forma negativa del verbo 'to be' para sujeto singular o plural."
  - "Aplica al sujeto 'you' correctamente."

respuestas_validas:
  - "are not"
  - "aren't"

tipo: completar

enunciado: "You ___ wrong. (Complete the negative form of 'to be'.)"

explicacion: "La forma negativa de 'you are' es 'are not' o su contracción 'aren't'."
```

### 18 — Contraction in Affirmative ('It's')

```
metadata:
  materia: "ingles"
  tema: "to-be"
  nivel: "A1"
  tags: ["contracción", "is"]

pasos:
  - "Identifica la contracción de 'it is'."
  - "Aplica al sujeto 'it' correctamente."

respuestas_validas:
  - "'s"

tipo: completar

enunciado: "It ___ important. (Complete with the contraction of 'is'.)"

explicacion: "La contracción de 'it is' es 'it's', que en este hueco se escribe simplemente 's (sin espacio antes)."
```

### 19 — Interrogative with 'Are They'

```
metadata:
  materia: "ingles"
  tema: "to-be"
  nivel: "A1"
  tags: ["interrogativa", "are"]

pasos:
  - "Reconoce la estructura de pregunta con el sujeto 'they'."
  - "Aplica la forma correcta del verbo 'to be' en presente."

respuestas_validas:
  - "Are they"
  - "are they"

tipo: completar

enunciado: "___ going to the store? (Complete the question with the correct form of 'to be'.)"

explicacion: "Para preguntar sobre el estado de 'they', se usa 'Are they'."
```

### 20 — Negative Form with 'I'

```
metadata:
  materia: "ingles"
  tema: "to-be"
  nivel: "A1"
  tags: ["negativo", "am"]

pasos:
  - "Reconoce la forma negativa del verbo 'to be' para sujeto singular."
  - "Aplica al sujeto 'I' correctamente."

respuestas_validas:
  - "am not"
  - "'m not"

tipo: completar

enunciado: "I ___ happy. (Complete the negative form of 'to be'.)"

explicacion: "La forma negativa de 'I am' es 'am not'; en habla informal se contrae como 'm not (\"I'm not\")."
```

### 21 — Contraction in Affirmative ('We're')

```
metadata:
  materia: "ingles"
  tema: "to-be"
  nivel: "A1"
  tags: ["contracción", "are"]

pasos:
  - "Identifica la contracción de 'we are'."
  - "Aplica al sujeto 'we' correctamente."

respuestas_validas:
  - "'re"

tipo: completar

enunciado: "We ___ going to meet. (Complete with the contraction of 'are'.)"

explicacion: "La contracción de 'we are' es 'we're', que en este hueco se escribe simplemente 're (sin espacio antes)."
```

### 22 — Interrogative with 'Am I'

```
metadata:
  materia: "ingles"
  tema: "to-be"
  nivel: "A1"
  tags: ["interrogativa", "am"]

pasos:
  - "Reconoce la estructura de pregunta con el sujeto 'I'."
  - "Aplica la forma correcta del verbo 'to be' en presente."

respuestas_validas:
  - "Am I"
  - "am I"

tipo: completar

enunciado: "___ your friend? (Complete the question with the correct form of 'to be'.)"

explicacion: "Para preguntar sobre el estado de 'I', se usa 'Am I'."
```

### 23 — Negative Form with 'She'

```
metadata:
  materia: "ingles"
  tema: "to-be"
  nivel: "A1"
  tags: ["negativo", "is"]

pasos:
  - "Reconoce la forma negativa del verbo 'to be' para sujeto singular."
  - "Aplica al sujeto 'she' correctamente."

respuestas_validas:
  - "is not"
  - "isn't"

tipo: completar

enunciado: "She ___ here. (Complete the negative form of 'to be'.)"

explicacion: "La forma negativa de 'she is' es 'is not' o su contracción 'isn't'."
```

### 24 — Contraction in Affirmative ('It's a car')

```
metadata:
  materia: "ingles"
  tema: "to-be"
  nivel: "A1"
  tags: ["contracción", "is"]

pasos:
  - "Identifica la contracción de 'it is'."
  - "Aplica al sujeto 'it' correctamente."

respuestas_validas:
  - "'s"

tipo: completar

enunciado: "It ___ a car. (Complete with the contraction of 'is'.)"

explicacion: "La contracción de 'it is' es 'it's', que en este hueco se escribe simplemente 's (sin espacio antes)."
```

### 25 — Interrogative with 'Are We' (Revised)

```
metadata:
  materia: "ingles"
  tema: "to-be"
  nivel: "A1"
  tags: ["interrogativa", "are"]

pasos:
  - "Reconoce la estructura de pregunta con el sujeto 'we'."
  - "Aplica la forma correcta del verbo 'to be' en presente."

respuestas_validas:
  - "Are we"
  - "are we"

tipo: completar

enunciado: "___ in the same class? (Complete the question with the correct form of 'to be'.)"

explicacion: "Para preguntar sobre el estado de 'we', se usa 'Are we'."
```
