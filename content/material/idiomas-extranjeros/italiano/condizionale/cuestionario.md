# Idiomas — Italiano — condizionale (cuestionario)

> Promovido desde el borrador (`../../../_borradores-gemma/idiomas-extranjeros/italiano/condizionale/cuestionario_crudo.md`), ya validaba 0 FAIL, sin cambios de contenido.

---

### 1 — Condizionale con verbi regolari  
```  
metadata:  
  materia: "idiomas-extranjeros/italiano"  
  tema: "condizionale"  
  nivel: "B1"  
  tags: ["verbi regolari", "struttura base"]  
pasos:  
  - "Identifica la forma corretta del verbo 'mangiare' nel condizionale."  
explicacion: "El condizionale de los verbos regulares se forma con el auxiliar 'avrei' + participio passato o con 'sarei' + infinitivo. En este caso, 'mangierei' es la forma correcta."  
tipo: completar  
enunciado: "Se avessi fame, ___________ una pizza."  
respuestas_validas:  
  - "mangierei"  
  - "Mangierei"  
```  

---

### 2 — Condizionale con verbo irregolare (avere)  
```  
metadata:  
  materia: "idiomas-extranjeros/italiano"  
  tema: "condizionale"  
  nivel: "B1"  
  tags: ["verbi irregolari", "avere"]  
pasos:  
  - "Completa la frase con la forma condizionale del verbo 'avere'."  
explicacion: "El condizionale de 'avere' es 'avrei', que se usa para hipótesis o deseos no realizados. En este contexto, 'avrei bisogno' expresa una necesidad hipotética."  
tipo: completar  
enunciado: "Se non ___________ tempo, non potrò aiutarti."  
respuestas_validas:  
  - "avessi"  
  - "Avessi"  
```  

---

### 3 — Condizionale con verbo irregolare (dovere)  
```  
metadata:  
  materia: "idiomas-extranjeros/italiano"  
  tema: "condizionale"  
  nivel: "B1"  
  tags: ["verbi irregolari", "dovere"]  
pasos:  
  - "Elige la forma correcta del verbo 'dovere' en condizionale."  
explicacion: "'Dovrei' es la forma condizionale de 'dovere', usada para expresar obligaciones hipotéticas o deseos suaves. En este contexto, se usa para una sugerencia."  
tipo: mc  
enunciado: "___________ andare con me?"  
opciones_explicitas:
  - "Dovrei"
  - "Doverei"
  - "Dovevo"
respuesta: "Dovrei"  
```  

---

### 4 — Condizionale con verbo irregolare (potere)  
```  
metadata:  
  materia: "idiomas-extranjeros/italiano"  
  tema: "condizionale"  
  nivel: "B1"  
  tags: ["verbi irregolari", "potere"]  
pasos:  
  - "Completa la frase con la forma condizionale de 'potere'."  
explicacion: "'Potrei' es la forma correcta del condizionale de 'potere', usada para expresar posibilidades hipotéticas o deseos no realizados."  
tipo: completar  
enunciado: "Se avessi più soldi, ___________ viaggiare di più."  
respuestas_validas:  
  - "potrei"  
  - "Potrei"  
```  

---

### 5 — Condizionale con verbo irregolare (sapere)  
```  
metadata:  
  materia: "idiomas-extranjeros/italiano"  
  tema: "condizionale"  
  nivel: "B1"  
  tags: ["verbi irregolari", "sapere"]  
pasos:  
  - "Elige la forma correcta del verbo 'sapere' en condizionale."  
explicacion: "'Saprei' es la forma condizionale de 'sapere', usada para expresar conocimientos hipotéticos o deseos no realizados. En este contexto, se refiere a una habilidad futura."  
tipo: mc  
enunciado: "___________ aiutarti se avessi tempo."  
opciones_explicitas:
  - "Saprei"
  - "Sarei"
  - "Sapevo"
respuesta: "Saprei"  
```  

---

### 6 — Condizionale con verbo irregolare (volere)  
```  
metadata:  
  materia: "idiomas-extranjeros/italiano"  
  tema: "condizionale"  
  nivel: "B1"  
  tags: ["verbi irregolari", "volere"]  
pasos:  
  - "Completa la frase con la forma condizionale de 'volere'."  
explicacion: "'Vorrei' es la forma correcta del condizionale de 'volere', usada para expresar deseos no realizados o preferencias suaves. En este contexto, expresa un deseo."  
tipo: completar  
enunciado: "___________ mangiare una torta adesso?"  
respuestas_validas:  
  - "vorrei"  
  - "Vorrei"  
```  

---

### 7 — Condizionale con verbo irregolare (dare)  
```  
metadata:  
  materia: "idiomas-extranjeros/italiano"  
  tema: "condizionale"  
  nivel: "B1"  
  tags: ["verbi irregolari", "dare"]  
pasos:  
  - "Elige la forma correcta del verbo 'dare' en condizionale."  
explicacion: "'Darei' es la forma condizionale de 'dare', usada para expresar acciones hipotéticas. En este contexto, se refiere a una acción futura no segura."  
tipo: mc  
enunciado: "___________ un regalo per il tuo compleanno?"  
opciones_explicitas:
  - "Darei"
  - "Darrei"
  - "Davo"
respuesta: "Darei"  
```  

---

### 8 — Condizionale con verbo irregolare (fare)  
```  
metadata:  
  materia: "idiomas-extranjeros/italiano"  
  tema: "condizionale"  
  nivel: "B1"  
  tags: ["verbi irregolari", "fare"]  
pasos:  
  - "Completa la frase con la forma condizionale de 'fare'."  
explicacion: "'Farei' es la forma correcta del condizionale de 'fare', usada para expresar acciones hipotéticas. En este contexto, expresa una acción futura no segura."  
tipo: completar  
enunciado: "Se avessi tempo libero, ___________ un viaggio in Giappone."  
respuestas_validas:  
  - "farei"  
  - "Farei"  
```  

---

### 9 — Condizionale con verbo irregolare (andare)  
```  
metadata:  
  materia: "idiomas-extranjeros/italiano"  
  tema: "condizionale"  
  nivel: "B1"  
  tags: ["verbi irregolari", "andare"]  
pasos:  
  - "Elige la forma correcta del verbo 'andare' en condizionale."  
explicacion: "'Andrei' es la forma condizionale de 'andare', usada para expresar movimientos hipotéticos. En este contexto, se refiere a un viaje futura no seguro."  
tipo: mc  
enunciado: "___________ in vacanza quest'estate?"  
opciones_explicitas:
  - "Andrei"
  - "Andassi"
  - "Andai"
respuesta: "Andrei"  
```  

---

### 10 — Condizionale con verbo irregolare (prendere)  
```  
metadata:  
  materia: "idiomas-extranjeros/italiano"  
  tema: "condizionale"  
  nivel: "B1"  
  tags: ["verbi irregolari", "prendere"]  
pasos:  
  - "Completa la frase con la forma condizionale de 'prendere'."  
explicacion: "'Prenderei' es la forma correcta del condizionale de 'prendere', usada para expresar acciones hipotéticas. En este contexto, expresa una acción futura no segura."  
tipo: completar  
enunciado: "Se avessi fame, ___________ un panino."  
respuestas_validas:  
  - "prenderei"  
  - "Prenderei"  
```  

---

### 11 — Condizionale con verbo irregolare (vedere)  
```  
metadata:  
  materia: "idiomas-extranjeros/italiano"  
  tema: "condizionale"  
  nivel: "B1"  
  tags: ["verbi irregolari", "vedere"]  
pasos:  
  - "Elige la forma correcta del verbo 'vedere' en condizionale."  
explicacion: "'Vederei' es la forma condizionale de 'vedere', usada para expresar acciones hipotéticas. En este contexto, se refiere a una acción futura no segura."  
tipo: mc  
enunciado: "___________ il film se avessi tempo?"  
opciones_explicitas:
  - "Vederei"
  - "Vedrei"
  - "Vedevo"
respuesta: "Vederei"  
```  

---

### 12 — Condizionale con verbo irregolare (credere)  
```  
metadata:  
  materia: "idiomas-extranjeros/italiano"  
  tema: "condizionale"  
  nivel: "B1"  
  tags: ["verbi irregolari", "credere"]  
pasos:  
  - "Completa la frase con la forma condizionale de 'credere'."  
explicacion: "'Crederò' no es correcto en condizionale. La forma correcta es 'crederesti', que se usa para hipótesis o deseos no realizados. En este contexto, expresa una creencia hipotética."  
tipo: completar  
enunciado: "Se fossi tu, ___________ che fosse giusto."  
respuestas_validas:  
  - "crederesti"  
  - "Crederesti"  
```  

---

### 13 — Condizionale con verbo irregolare (dire)  
```  
metadata:  
  materia: "idiomas-extranjeros/italiano"  
  tema: "condizionale"  
  nivel: "B1"  
  tags: ["verbi irregolari", "dire"]  
pasos:  
  - "Elige la forma correcta del verbo 'dire' en condizionale."  
explicacion: "'Direi' es la forma condizionale de 'dire', usada para expresar acciones hipotéticas. En este contexto, se refiere a una acción futura no segura."  
tipo: mc  
enunciado: "___________ ciò che penso?"  
opciones_explicitas:
  - "Direi"
  - "Dissi"
  - "Dirò"
respuesta: "Direi"  
```  

---

### 14 — Condizionale con verbo irregolare (bere)  
```  
metadata:  
  materia: "idiomas-extranjeros/italiano"  
  tema: "condizionale"  
  nivel: "B1"  
  tags: ["verbi irregolari", "bere"]  
pasos:  
  - "Completa la frase con la forma condizionale de 'bere'."  
explicacion: "'Berei' es la forma correcta del condizionale de 'bere', usada para expresar acciones hipotéticas. En este contexto, expresa una acción futura no segura."  
tipo: completar  
enunciado: "Se avessi sete, ___________ un bicchiere d'acqua."  
respuestas_validas:  
  - "berei"  
  - "Berei"  
```  

---

### 15 — Condizionale con verbo irregolare (uscire)  
```  
metadata:  
  materia: "idiomas-extranjeros/italiano"  
  tema: "condizionale"  
  nivel: "B1"  
  tags: ["verbi irregolari", "uscire"]  
pasos:  
  - "Elige la forma correcta del verbo 'uscire' en condizionale."  
explicacion: "'Uscirei' es la forma condizionale de 'uscire', usada para expresar acciones hipotéticas. En este contexto, se refiere a un viaje futura no seguro."  
tipo: mc  
enunciado: "___________ fuori per una passeggiata?"  
opciones_explicitas:
  - "Uscirei"
  - "Uscii"
  - "Uscirò"
respuesta: "Uscirei"  
```  

---

### 16 — Condizionale con verbo irregolare (venire)  
```  
metadata:  
  materia: "idiomas-extranjeros/italiano"  
  tema: "condizionale"  
  nivel: "B1"  
  tags: ["verbi irregolari", "venire"]  
pasos:  
  - "Completa la frase con la forma condizionale de 'venire'."  
explicacion: "'Venni' no es correcto en condizionale. La forma correcta es 'verresti', que se usa para hipótesis o deseos no realizados. En este contexto, expresa una acción futura no segura."  
tipo: completar  
enunciado: "Se fossi in città, ___________ a trovarmi."  
respuestas_validas:  
  - "verresti"  
  - "Verresti"  
```  

---

### 17 — Condizionale con verbo irregolare (stare)  
```  
metadata:  
  materia: "idiomas-extranjeros/italiano"  
  tema: "condizionale"  
  nivel: "B1"  
  tags: ["verbi irregolari", "stare"]  
pasos:  
  - "Elige la forma correcta del verbo 'stare' en condizionale."  
explicacion: "'Starei' es la forma condizionale de 'stare', usada para expresar estados hipotéticos. En este contexto, se refiere a un estado futuro no seguro."  
tipo: mc  
enunciado: "___________ qui domani?"  
opciones_explicitas:
  - "Starei"
  - "Stai"
  - "Stavo"
respuesta: "Starei"  
```  

---

### 18 — Condizionale con verbo irregolare (parlare)  
```  
metadata:  
  materia: "idiomas-extranjeros/italiano"  
  tema: "condizionale"  
  nivel: "B1"  
  tags: ["verbi irregolari", "parlare"]  
pasos:  
  - "Completa la frase con la forma condizionale de 'parlare'."  
explicacion: "'Parlerò' no es correcto en condizionale. La forma correcta es 'parleresti', que se usa para hipótesis o deseos no realizados. En este contexto, expresa una acción futura no segura."  
tipo: completar  
enunciado: "Se fossi in grado, ___________ con lui."  
respuestas_validas:  
  - "parleresti"  
  - "Parleresti"  
```  

---

### 19 — Condizionale con verbo irregolare (mangiare)  
```  
metadata:  
  materia: "idiomas-extranjeros/italiano"  
  tema: "condizionale"  
  nivel: "B1"  
  tags: ["verbi irregolari", "mangiare"]  
pasos:  
  - "Elige la forma correcta del verbo 'mangiare' en condizionale."  
explicacion: "'Mangerei' es la forma condizionale de 'mangiare', usada para expresar acciones hipotéticas. En este contexto, se refiere a una acción futura no segura."  
tipo: mc  
enunciado: "___________ una pizza?"  
opciones_explicitas:
  - "Mangerei"
  - "Mangerò"
  - "Mangi"
respuesta: "Mangerei"  
```  

---

### 20 — Condizionale con verbo irregolare (dormire)  
```  
metadata:  
  materia: "idiomas-extranjeros/italiano"  
  tema: "condizionale"  
  nivel: "B1"  
  tags: ["verbi irregolari", "dormire"]  
pasos:  
  - "Completa la frase con la forma condizionale de 'dormire'."  
explicacion: "'Dormirei' es la forma correcta del condizionale de 'dormire', usada para expresar acciones hipotéticas. En este contexto, expresa una acción futura no segura."  
tipo: completar  
enunciado: "Se avessi tempo libero, ___________ un sonnellino."  
respuestas_validas:  
  - "dormirei"  
  - "Dormirei"  
```  

---

### 21 — Condizionale con verbo irregolare (uscire)  
```  
metadata:  
  materia: "idiomas-extranjeros/italiano"  
  tema: "condizionale"  
  nivel: "B1"  
  tags: ["verbi irregolari", "uscire"]  
pasos:  
  - "Elige la forma correcta del verbo 'uscire' en condizionale."  
explicacion: "'Uscirò' no es correcto en condizionale. La forma correcta es 'usciresti', que se usa para hipótesis o deseos no realizados. En este contexto, expresa una acción futura no segura."  
tipo: mc  
enunciado: "___________ con me?"  
opciones_explicitas:
  - "Usciresti"
  - "Uscii"
  - "Uscirò"
respuesta: "Usciresti"  
```  

---

### 22 — Condizionale con verbo irregolare (rimanere)  
```  
metadata:  
  materia: "idiomas-extranjeros/italiano"  
  tema: "condizionale"  
  nivel: "B1"  
  tags: ["verbi irregolari", "rimanere"]  
pasos:  
  - "Completa la frase con la forma condizionale de 'rimanere'."  
explicacion: "'Rimarrei' es la forma correcta del condizionale de 'rimanere', usada para expresar estados hipotéticos. En este contexto, se refiere a un estado futuro no seguro."  
tipo: completar  
enunciado: "Se non avessi lavoro, ___________ a casa tutto il giorno."  
respuestas_validas:  
  - "rimarrei"  
  - "Rimarrei"  
```  

---

### 23 — Condizionale con verbo irregolare (finire)  
```  
metadata:  
  materia: "idiomas-extranjeros/italiano"  
  tema: "condizionale"  
  nivel: "B1"  
  tags: ["verbi irregolari", "finire"]  
pasos:  
  - "Elige la forma correcta del verbo 'finire' en condizionale."  
explicacion: "'Finirei' es la forma condizionale de 'finire', usada para expresar acciones hipotéticas. En este contexto, se refiere a una acción futura no segura."  
tipo: mc  
enunciado: "___________ il lavoro?"  
opciones_explicitas:
  - "Finirei"
  - "Finito"
  - "Finirò"
respuesta: "Finirei"  
```  

---

### 24 — Condizionale con verbo irregolare (andare)  
```  
metadata:  
  materia: "idiomas-extranjeros/italiano"  
  tema: "condizionale"  
  nivel: "B1"  
  tags: ["verbi irregolari", "andare"]  
pasos:  
  - "Completa la frase con la forma condizionale de 'andare'."  
explicacion: "'Andrei' es la forma correcta del condizionale de 'andare', usada para expresar movimientos hipotéticos. En este contexto, se refiere a un viaje futuro no seguro."  
tipo: completar  
enunciado: "Se avessi una macchina, ___________ in vacanza domani."  
respuestas_validas:  
  - "andrei"  
  - "Andrei"  
```  

---

### 25 — Condizionale con verbo irregolare (credere)  
```  
metadata:  
  materia: "idiomas-extranjeros/italiano"  
  tema: "condizionale"  
  nivel: "B1"  
  tags: ["verbi irregolari", "credere"]  
pasos:  
  - "Elige la forma correcta del verbo 'credere' en condizionale."  
explicacion: "'Crederò' no es correcto en condizionale. La forma correcta es 'crederesti', que se usa para hipótesis o deseos no realizados. En este contexto, expresa una creencia hipotética."  
tipo: mc  
enunciado: "___________ che fosse giusto?"  
opciones_explicitas:
  - "Crederesti"
  - "Credetti"
  - "Credo"
respuesta: "Crederesti"  
```
