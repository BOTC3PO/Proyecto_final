# Idiomas — Italiano — nominalizzazione (cuestionario)

> Promovido desde el borrador (`../../../_borradores-gemma/idiomas-extranjeros/italiano/nominalizzazione/cuestionario_crudo.md`)
> con únicamente correcciones mecánicas (comillas mal cerradas, `pasos:` sin comillas, líneas
> `---` sueltas dentro de un fence) vía `content/material/_qa_tools/mechanical_fix_idiomas.py`,
> sin cambios de contenido.

---

### 1 — Nominalizzazione di un verbo  
```
metadata:  
  materia: "idiomas-extranjeros/italiano"  
  tema: "nominalizzazione"  
  nivel: "C1"  
  tags: ["verbi", "-zione"]  
pasos:  
  - "Identificar el sufijo apropiado para transformar 'parlare' en un sustantivo."  
explicacion: "El verbo 'parlare' se nominaliza con el sufijo '-zione', formando 'discussione' (feminino). Este proceso es común para verbos terminados en -are."  
enunciado: "La ______ della politica è spesso controversa."  
tipo: completar  
respuestas_validas:  
  - "discussione"  
  - "DISCUSSIONE"  

```



### 2 — Nominalizzazione con sufijo -tura  
```
metadata:  
  materia: "idiomas-extranjeros/italiano"  
  tema: "nominalizzazione"  
  nivel: "C1"  
  tags: ["verbi", "-tura"]  
pasos:  
  - "Aplicar el sufijo '-tura' al verbo 'sentire'."  
explicacion: "El verbo 'sentire' (sentir) se convierte en 'sensazione' con el sufijo '-zione', no '-tura'. El sufijo '-tura' se usa para verbos terminados en -ere/-ire, como 'vedere' → 'visione'."  
enunciado: "La ______ del paesaggio era intensa."  
tipo: completar  
respuestas_validas:  
  - "visione"  
  - "VISIONE"  

```



### 3 — Elegir la forma nominal correcta  
```
metadata:  
  materia: "idiomas-extranjeros/italiano"  
  tema: "nominalizzazione"  
  nivel: "C1"  
  tags: ["opciones", "-zione"]  
pasos:  
  - "Seleccionar el sustantivo derivado de 'lavorare'."  
explicacion: "El verbo 'lavorare' se nominaliza como 'lavoro' (masculino singular), no 'lavorazione' o 'lavori', que son formas incorrectas para este proceso."  
enunciado: "Il ______ è un lavoro che richiede attenzione."  
tipo: mc  
opciones_explicitas:  
  - "lavoro"  
  - "lavorazione"  
  - "lavori"  
respuesta: "lavoro"  

```



### 4 — Nominalización de verbo en contexto  
```
metadata:  
  materia: "idiomas-extranjeros/italiano"  
  tema: "nominalizzazione"  
  nivel: "C1"  
  tags: ["contexto", "-zione"]  
pasos:  
  - "Identificar el sustantivo derivado de 'giocare' en la oración."  
explicacion: "El verbo 'giocare' (jugar) se transforma en 'gioco' (masculino singular), que es un sustantivo común usado en contextos como juegos o competencias."  
enunciado: "Il ______ preferito da bambini è il calcio."  
tipo: completar  
respuestas_validas:  
  - "gioco"  
  - "GIUOCO"  

```



### 5 — Elegir forma nominal para verbo en -ire  
```
metadata:  
  materia: "idiomas-extranjeros/italiano"  
  tema: "nominalizzazione"  
  nivel: "C1"  
  tags: ["verbi", "-tura"]  
pasos:  
  - "Seleccionar el sufijo correcto para 'aprire'."  
explicacion: "Los verbos en -ire como 'aprire' se nominalizan con '-tura', formando 'apertura'. Otros ejemplos son 'chiudere' → 'chiusura'."  
enunciado: "La ______ del documento è necessaria per il deposito."  
tipo: completar  
respuestas_validas:  
  - "apertura"  
  - "APERTURA"  

```



### 6 — Nominalización de verbo abstracto  
```
metadata:  
  materia: "idiomas-extranjeros/italiano"  
  tema: "nominalizzazione"  
  nivel: "C1"  
  tags: ["verbi", "-zione"]  
pasos:  
  - "Transformar 'conoscere' en un sustantivo abstracto."  
explicacion: "El verbo 'conoscere' (conocer) se convierte en 'conoscenza' con el sufijo '-enza', que se usa para verbos terminados en -cere, -scere, etc."  
enunciado: "La ______ del nuovo sistema richiede tempo."  
tipo: completar  
respuestas_validas:  
  - "conoscenza"  
  - "CONOSCENZA"  

```



### 7 — Elegir forma nominal de verbo en -ere  
```
metadata:  
  materia: "idiomas-extranjeros/italiano"  
  tema: "nominalizzazione"  
  nivel: "C1"  
  tags: ["verbi", "-tura"]  
pasos:  
  - "Seleccionar el sustantivo derivado de 'vedere'."  
explicacion: "El verbo 'vedere' (ver) se nominaliza como 'visione' con '-zione', no '-tura'. El sufijo '-tura' es para verbos en -ire, como 'chiudere' → 'chiusura'."  
enunciado: "La ______ del film è stata eccezionale."  
tipo: mc  
opciones_explicitas:  
  - "visione"  
  - "vista"  
  - "veduta"  
respuesta: "visione"  

```



### 8 — Nominalización de verbo con sufijo -sione  
```
metadata:  
  materia: "idiomas-extranjeros/italiano"  
  tema: "nominalizzazione"  
  nivel: "C1"  
  tags: ["verbi", "-sione"]  
pasos:  
  - "Identificar el sustantivo derivado de 'rispondere'."  
explicacion: "El verbo 'rispondere' (responder) se convierte en 'risposta' con '-sione', que es la forma correcta para verbos terminados en -dare o -dere."  
enunciado: "La ______ al questionario era chiara e precisa."  
tipo: completar  
respuestas_validas:  
  - "risposta"  
  - "RISPOSTA"  

```



### 9 — Elegir forma nominal de verbo en -ire  
```
metadata:  
  materia: "idiomas-extranjeros/italiano"  
  tema: "nominalizzazione"  
  nivel: "C1"  
  tags: ["verbi", "-tura"]  
pasos:  
  - "Seleccionar el sustantivo derivado de 'dormire'."  
explicacion: "El verbo 'dormire' (dormir) se nominaliza como 'sonno' con '-sione', no '-tura'. El sufijo '-tura' se usa para verbos en -ire, pero 'dormire' es una excepción que toma '-sione'."  
enunciado: "Il ______ di un bambino è profondo e rilassante."  
tipo: completar  
respuestas_validas:  
  - "sonno"  
  - "SOMNO"  

```



### 10 — Nominalización de verbo abstracto con -enza  
```
metadata:  
  materia: "idiomas-extranjeros/italiano"  
  tema: "nominalizzazione"  
  nivel: "C1"  
  tags: ["verbi", "-enza"]  
pasos:  
  - "Transformar 'creare' en un sustantivo abstracto."  
explicacion: "El verbo 'creare' (crear) se convierte en 'creazione' con '-zione', no '-enza'. El sufijo '-enza' se usa para verbos terminados en -cere, como 'conoscere' → 'conoscenza'."  
enunciado: "La ______ dell’opera d’arte è unica e originale."  
tipo: completar  
respuestas_validas:  
  - "creazione"  
  - "CREAZIONE"  

```



### 11 — Elegir forma nominal de verbo en -are  
```
metadata:  
  materia: "idiomas-extranjeros/italiano"  
  tema: "nominalizzazione"  
  nivel: "C1"  
  tags: ["verbi", "-zione"]  
pasos:  
  - "Seleccionar el sustantivo derivado de 'correre'."  
explicacion: "El verbo 'correre' (correr) se nominaliza como 'corsa' con '-sione', que es la forma correcta para verbos terminados en -rre, -cere, etc."  
enunciado: "La ______ del gatto era veloce e silenziosa."  
tipo: mc  
opciones_explicitas:  
  - "corsa"  
  - "corso"  
  - "corrida"  
respuesta: "corsa"  

```



### 12 — Nominalización de verbo con sufijo -zione  
```
metadata:  
  materia: "idiomas-extranjeros/italiano"  
  tema: "nominalizzazione"  
  nivel: "C1"  
  tags: ["verbi", "-zione"]  
pasos:  
  - "Identificar el sustantivo derivado de 'parlare' en contexto."  
explicacion: "El verbo 'parlare' (hablar) se convierte en 'discussione' con '-zione', que es la forma correcta para verbos terminados en -are."  
enunciado: "La ______ tra i due amici era sempre intensa."  
tipo: completar  
respuestas_validas:  
  - "discussione"  
  - "DISCUSSIONE"  

```



### 13 — Elegir forma nominal de verbo en -ire con -tura  
```
metadata:  
  materia: "idiomas-extranjeros/italiano"  
  tema: "nominalizzazione"  
  nivel: "C1"  
  tags: ["verbi", "-tura"]  
pasos:  
  - "Seleccionar el sustantivo derivado de 'chiudere'."  
explicacion: "El verbo 'chiudere' (cerrar) se nominaliza como 'chiusura' con '-tura', que es la forma correcta para verbos terminados en -ire."  
enunciado: "La ______ del portone era un problema urgente."  
tipo: completar  
respuestas_validas:  
  - "chiusura"  
  - "CHIUSURA"  

```



### 14 — Nominalización de verbo abstracto con -sione  
```
metadata:  
  materia: "idiomas-extranjeros/italiano"  
  tema: "nominalizzazione"  
  nivel: "C1"  
  tags: ["verbi", "-sione"]  
pasos:  
  - "Transformar 'dormire' en un sustantivo abstracto."  
explicacion: "El verbo 'dormire' (dormir) se convierte en 'sonno' con '-sione', que es la forma correcta para verbos terminados en -mire, -mirare, etc."  
enunciado: "Il ______ di una notte tranquilla è raro in città."  
tipo: completar  
respuestas_validas:  
  - "sonno"  
  - "SOMNO"  

```



### 15 — Elegir forma nominal de verbo en -are con -zione  
```
metadata:  
  materia: "idiomas-extranjeros/italiano"  
  tema: "nominalizzazione"  
  nivel: "C1"  
  tags: ["verbi", "-zione"]  
pasos:  
  - "Seleccionar el sustantivo derivado de 'scrivere'."  
explicacion: "El verbo 'scrivere' (escribir) se nominaliza como 'scrittura' con '-tura', no '-zione'. El sufijo '-zione' se usa para verbos terminados en -are, pero 'scrivere' es una excepción que toma '-tura'."  
enunciado: "La ______ del contratto deve essere verificata."  
tipo: mc  
opciones_explicitas:  
  - "scrittura"  
  - "scrizione"  
  - "scrittura"  
respuesta: "scrittura"  

```



### 16 — Nominalización de verbo con sufijo -zione en contexto  
```
metadata:  
  materia: "idiomas-extranjeros/italiano"  
  tema: "nominalizzazione"  
  nivel: "C1"  
  tags: ["verbi", "-zione"]  
pasos:  
  - "Identificar el sustantivo derivado de 'giocare' en la oración."  
explicacion: "El verbo 'giocare' (jugar) se convierte en 'gioco' con '-co', que es una excepción a las reglas generales de nominalización, pero se usa comúnmente para este verbo."  
enunciado: "Il ______ preferito dai bambini è il calcio."  
tipo: completar  
respuestas_validas:  
  - "gioco"  
  - "GIUOCO"  

```



### 17 — Elegir forma nominal de verbo en -ere con -zione  
```
metadata:  
  materia: "idiomas-extranjeros/italiano"  
  tema: "nominalizzazione"  
  nivel: "C1"  
  tags: ["verbi", "-zione"]  
pasos:  
  - "Seleccionar el sustantivo derivado de 'vedere'."  
explicacion: "El verbo 'vedere' (ver) se nominaliza como 'visione' con '-sione', no '-tura'. El sufijo '-tura' es para verbos en -ire, pero 'vedere' toma '-zione'."  
enunciado: "La ______ del film era eccezionale."  
tipo: completar  
respuestas_validas:  
  - "visione"  
  - "VISIONE"  

```



### 18 — Nominalización de verbo abstracto con -sione  
```
metadata:  
  materia: "idiomas-extranjeros/italiano"  
  tema: "nominalizzazione"  
  nivel: "C1"  
  tags: ["verbi", "-sione"]  
pasos:  
  - "Transformar 'parlare' en un sustantivo abstracto."  
explicacion: "El verbo 'parlare' (hablar) se convierte en 'discussione' con '-zione', que es la forma correcta para verbos terminados en -are."  
enunciado: "La ______ tra i due gruppi era intensa e polemica."  
tipo: completar  
respuestas_validas:  
  - "discussione"  
  - "DISCUSSIONE"  

```



### 19 — Elegir forma nominal de verbo en -ire con -tura  
```  
metadata:  
  materia: "italiano"  
  tema: "nominalizzazione"  
  nivel: "C1"  
  tags: ["verbo", "processo"]  
pasos:  
  - "Identificar el verbo en la oración y transformarlo en forma nominal."  
  - "Verificar que el sustantivo coincida con el contexto del proceso descrito."  
tipo: completar  
respuestas_validas:  
  - "Implementazione"  
  - "IMPLEMENTAZIONE"  
enunciado: "La ______ del nuovo sistema informatico richiede tempo e risorse."  
explicacion: "El verbo 'implementare' se convierte en el sustantivo 'implementazione', que refiere al proceso de poner en práctica un sistema. Otros términos como 'esecuzione' o 'attuazione' no son válidos aquí por contexto."  
```

### 20 — Nominalización de verbo con sufijo -zione en contexto  
```  
metadata:  
  materia: "italiano"  
  tema: "nominalizzazione"  
  nivel: "C1"  
  tags: ["verbo", "acción"]  
pasos:  
  - "Identificar el verbo en la oración y encontrar su forma nominal."  
  - "Asegurar que el sustantivo refleje una acción colectiva o social."  
tipo: completar  
respuestas_validas:  
  - "Solidarietà"  
  - "SOLIDARIETÀ"  
enunciado: "La ______ tra i volontari era un valore fondamentale per l’associazione."  
explicacion: "'Essere solidali' se transforma en 'solidarietà', que denota un valor colectivo. Opciones como 'collaborazione' o 'aiuto' son incorrectas por contexto semántico."  
```

### 21 — Elegir forma nominal de verbo en -are con -zione  
```  
metadata:  
  materia: "italiano"  
  tema: "nominalizzazione"  
  nivel: "C1"  
  tags: ["verbo", "emoción"]  
pasos:  
  - "Reconocer el verbo que describe una emoción y convertirlo en sustantivo."  
  - "Verificar que el término coincida con la descripción de la emoción en la oración."  
tipo: completar  
respuestas_validas:  
  - "Felicità"  
  - "FELICITÀ"  
enunciado: "La ______ del bambino fu visibile nel suo sorriso."  
explicacion: "'Sentire felicità' se convierte en 'felicità', que denota un estado emocional. Términos como 'gioia' o 'allegria' son sinónimos pero no coinciden con la forma nominal requerida."  
```

### 22 — Nominalización de verbo con sufijo -tura  
```  
metadata:  
  materia: "italiano"  
  tema: "nominalizzazione"  
  nivel: "C1"  
  tags: ["verbo", "decisión"]  
pasos:  
  - "Identificar el verbo relacionado con la toma de decisiones y encontrar su forma nominal."  
  - "Asegurar que el sustantivo refleje un acto colectivo o institucional."  
tipo: completar  
respuestas_validas:  
  - "Decisione"  
  - "DECISIONE"  
enunciado: "La ______ del governo fu contestata da molti cittadini."  
explicacion: "'Prendere una decisione' se convierte en 'decisione', que denota un acto institucional. Opciones como 'parlamento' o 'voto' son incorrectas por contexto gramatical y semántico."  
```

### 23 — Elegir forma nominal de verbo en -ire con -tura  
```  
metadata:  
  materia: "italiano"  
  tema: "nominalizzazione"  
  nivel: "C1"  
  tags: ["verbo", "relación"]  
pasos:  
  - "Identificar el verbo que describe una relación y convertirlo en sustantivo."  
  - "Verificar que el término se alinee con la descripción de la relación en la oración."  
tipo: completar  
respuestas_validas:  
  - "Comprensione"  
  - "COMPRENSIONE"  
enunciado: "La ______ tra i due protagonisti fu chiave per risolvere il conflitto."  
explicacion: "'Comprendere' se convierte en 'comprensione', que denota una relación de entendimiento. Términos como 'dialogo' o 'accordo' son incorrectos por contexto gramatical y semántico."  
```

### 24 — Nominalización de verbo con sufijo -sione  
```  
metadata:  
  materia: "italiano"  
  tema: "nominalizzazione"  
  nivel: "C1"  
  tags: ["verbo", "fenómeno"]  
pasos:  
  - "Identificar el verbo que describe un fenómeno natural y convertirlo en sustantivo."  
  - "Asegurar que el término coincida con la descripción del fenómeno en la oración."  
tipo: completar  
respuestas_validas:  
  - "Tempesta"  
  - "TEMPESTA"  
enunciado: "La ______ del mare fu causata da un forte vento meridionale."  
explicacion: "'Scatenare una tempesta' se convierte en 'tempesta', que denota un fenómeno natural. Opciones como 'uragano' o 'onda' son incorrectas por contexto semántico y gramatical."  
```

### 25 — Elegir forma nominal de verbo en -are con -zione  
```  
metadata:  
  materia: "italiano"  
  tema: "nominalizzazione"  
  nivel: "C1"  
  tags: ["verbo", "cambio"]  
pasos:  
  - "Identificar el verbo que describe un cambio y convertirlo en sustantivo."  
  - "Verificar que el término refleje una transformación social o cultural."  
tipo: completar  
respuestas_validas:  
  - "Trasformazione"  
  - "TRASFORMAZIONE"  
enunciado: "La ______ delle abitudini alimentari fu un risultato del nuovo programma dietetico."  
explicacion: "'Trasformare' se convierte en 'trasformazione', que denota un cambio en hábitos. Términos como 'cambiamento' o 'modifica' son incorrectos por contexto gramatical y semántico."  
```
