# Idiomas — Italiano — condizionale-passato (cuestionario)

> Promovido desde el borrador (`../../../_borradores-gemma/idiomas-extranjeros/italiano/condizionale-passato/cuestionario_crudo.md`)
> con una corrección mecánica: tres bloques tenían comillas dobles anidadas dentro de un ítem de
> `pasos:` ya entrecomillado, que cerraban la cadena antes de tiempo; se cambiaron las comillas
> internas a simples.

---

### 1 — Condizionale passato con "essere"  
```
metadata:  
  materia: "idiomas-extranjeros/italiano"  
  tema: "condizionale-passato"  
  nivel: "B2"  
  tags: ["essere", "ipotesi"]  
pasos:  
  - "Identificar el verbo principal y su conjugación en condizionale passato."
  - "Verificar la concordancia de tiempo y modo con la subordinada."
tipo: completar  
respuestas_validas:  
  - "sarei stato"  
  - "Sarei stato"  
enunciado: "Se non ___ (essere) più attento, avrei commesso un errore importante."
```

### 2 — Condizionale passato con "avere" e participio  
```
metadata:  
  materia: "idiomas-extranjeros/italiano"  
  tema: "condizionale-passato"  
  nivel: "B2"  
  tags: ["avere", "participio"]  
pasos:  
  - "Formar el condizionale passato con 'avere' + participio."
  - "Asegurar que el participio concuerde en género y número con el sujeto."
tipo: completar  
respuestas_validas:  
  - "avrei avuto"  
  - "Avrei avuto"  
enunciado: "Se ___ (avere) più soldi, sarei andato in vacanza."
```

### 3 — Condizionale passato en oración hipotética  
```
metadata:  
  materia: "idiomas-extranjeros/italiano"  
  tema: "condizionale-passato"  
  nivel: "B2"  
  tags: ["ipotesi", "subordinada"]  
pasos:  
  - "Identificar la oración principal y subordinada."
  - "Aplicar el condizionale passato a la subordinada hipotética."
tipo: completar  
respuestas_validas:  
  - "saremmo andati"  
  - "Saremmo andati"  
enunciado: "Se non ___ (andare) in ufficio, avremmo perso l’opportunità."
```

### 4 — Opción correcta para condizionale passato  
```
metadata:  
  materia: "idiomas-extranjeros/italiano"  
  tema: "condizionale-passato"  
  nivel: "B2"  
  tags: ["mc", "verbo esserci"]  
pasos:  
  - "Analizar las opciones y seleccionar la forma correcta del condizionale passato."
respuesta: "saremmo stati"  
tipo: mc  
opciones_explicitas:  
  - "saremmo stati"  
  - "siamo stati"  
  - "sarei stato"  
enunciado: "Se ___ (essere) presenti, avremmo risolto il problema."
```

### 5 — Condizionale passato con participio compuesto  
```
metadata:  
  materia: "idiomas-extranjeros/italiano"  
  tema: "condizionale-passato"  
  nivel: "B2"  
  tags: ["participio", "avere"]  
pasos:  
  - "Formar el condizionale passato con participio compuesto."
  - "Verificar el sujeto y el verbo auxiliar."
tipo: completar  
respuestas_validas:  
  - "avrei preparato"  
  - "Avrei preparato"  
enunciado: "Se ___ (preparare) i documenti, non avremmo avuto problemi."
```

### 6 — Condizionale passato en oración de causa  
```
metadata:  
  materia: "idiomas-extranjeros/italiano"  
  tema: "condizionale-passato"  
  nivel: "B2"  
  tags: ["causa", "subordinada"]  
pasos:  
  - "Identificar la relación causal entre las oraciones."
  - "Aplicar el condizionale passato en la subordinada."
tipo: completar  
respuestas_validas:  
  - "avrei ascoltato"  
  - "Avrei ascoltato"  
enunciado: "Se ___ (ascoltare) le istruzioni, non avremmo sbagliato."
```

### 7 — Condizionale passato con verbo irregolare  
```
metadata:  
  materia: "idiomas-extranjeros/italiano"  
  tema: "condizionale-passato"  
  nivel: "B2"  
  tags: ["verbo irregular", "avere"]  
pasos:  
  - "Usar el condizionale passato con un verbo irregolare."
  - "Verificar la forma del participio."
tipo: completar  
respuestas_validas:  
  - "avrei potuto"  
  - "Avrei potuto"  
enunciado: "Se ___ (potere) aiutarti, lo avrei fatto."
```

### 8 — Condizionale passato con subordinada hipotética  
```
metadata:  
  materia: "idiomas-extranjeros/italiano"  
  tema: "condizionale-passato"  
  nivel: "B2"  
  tags: ["subordinada", "ipotesi"]  
pasos:  
  - "Identificar el tipo de oración subordinada."
  - "Aplicar el condizionale passato correctamente."
tipo: completar  
respuestas_validas:  
  - "sarebbe rimasto"  
  - "Sarebbe rimasto"  
enunciado: "Se ___ (rimanere) a casa, non avremmo avuto problemi."
```

### 9 — Condizionale passato con verbo reflexivo  
```
metadata:  
  materia: "idiomas-extranjeros/italiano"  
  tema: "condizionale-passato"  
  nivel: "B2"  
  tags: ["reflexivo", "avere"]  
pasos:  
  - "Formar el condizionale passato con verbo reflexivo."
  - "Verificar el sujeto y el participio."
tipo: completar  
respuestas_validas:  
  - "mi sarei lavato"  
  - "Mi sarei lavato"  
enunciado: "Se ___ (lavarsi), non avrei preso freddo."
```

### 10 — Condizionale passato con "essere" y participio  
```
metadata:  
  materia: "idiomas-extranjeros/italiano"  
  tema: "condizionale-passato"  
  nivel: "B2"  
  tags: ["essere", "participio"]  
pasos:  
  - "Usar el condizionale passato con verbo auxiliar 'essere'."
  - "Asegurar concordancia de género y número."
tipo: completar  
respuestas_validas:  
  - "sarei stato"  
  - "Sarei stato"  
enunciado: "Se ___ (essere) più coraggioso, avrei agito diversamente."
```

### 11 — Condizionale passato en oración de condición  
```
metadata:  
  materia: "idiomas-extranjeros/italiano"  
  tema: "condizionale-passato"  
  nivel: "B2"  
  tags: ["condición", "subordinada"]  
pasos:  
  - "Identificar la oración de condición."
  - "Aplicar el condizionale passato a la subordinada."
tipo: completar  
respuestas_validas:  
  - "avrei visto"  
  - "Avrei visto"  
enunciado: "Se ___ (vedere) il film, non l’avrei mai criticato."
```

### 12 — Condizionale passato con verbo compuesto  
```
metadata:  
  materia: "idiomas-extranjeros/italiano"  
  tema: "condizionale-passato"  
  nivel: "B2"  
  tags: ["verbo composto", "avere"]  
pasos:  
  - "Formar el condizionale passato con verbo compuesto."
  - "Verificar la forma del participio y el auxiliar."
tipo: completar  
respuestas_validas:  
  - "sarei andato"  
  - "Sarei andato"  
enunciado: "Se ___ (andare) in tempo, non avremmo perso il treno."
```

### 13 — Condizionale passato en oración de negación  
```
metadata:  
  materia: "idiomas-extranjeros/italiano"  
  tema: "condizionale-passato"  
  nivel: "B2"  
  tags: ["negazione", "subordinada"]  
pasos:  
  - "Analizar la estructura de negación en oraciones hipotéticas."
  - "Aplicar el condizionale passato correctamente."
tipo: completar  
respuestas_validas:  
  - "non sarei andato"  
  - "Non sarei andato"  
enunciado: "Se ___ (andare) al ristorante, non avrei mangiato."
```

### 14 — Condizionale passato con verbo impersonale  
```
metadata:  
  materia: "idiomas-extranjeros/italiano"  
  tema: "condizionale-passato"  
  nivel: "B2"  
  tags: ["impersonale", "avere"]  
pasos:  
  - "Usar el condizionale passato en oraciones impersonales."
  - "Verificar la forma del verbo auxiliar y participio."
tipo: completar  
respuestas_validas:  
  - "si sarebbe dovuto"  
  - "Si sarebbe dovuto"  
enunciado: "Se ___ (dovere) aspettare, non saremmo partiti."
```

### 15 — Condizionale passato con verbo modal  
```
metadata:  
  materia: "idiomas-extranjeros/italiano"  
  tema: "condizionale-passato"  
  nivel: "B2"  
  tags: ["modal", "avere"]  
pasos:  
  - "Formar el condizionale passato con verbo modal."
  - "Verificar la concordancia del participio."
tipo: completar  
respuestas_validas:  
  - "avrei potuto"  
  - "Avrei potuto"  
enunciado: "Se ___ (potere) aiutarti, lo avrei fatto."
```

### 16 — Condizionale passato con participio irregolare  
```
metadata:  
  materia: "idiomas-extranjeros/italiano"  
  tema: "condizionale-passato"  
  nivel: "B2"  
  tags: ["participio irregular", "avere"]  
pasos:  
  - "Usar el condizionale passato con participios irregulares."
  - "Verificar la forma correcta del participio."
tipo: completar  
respuestas_validas:  
  - "avrei voluto"  
  - "Avrei voluto"  
enunciado: "Se ___ (volere) parlare, non l’avrei mai fatto."
```

### 17 — Condizionale passato en oración de duda  
```
metadata:  
  materia: "idiomas-extranjeros/italiano"  
  tema: "condizionale-passato"  
  nivel: "B2"  
  tags: ["duda", "subordinada"]  
pasos:  
  - "Identificar la oración de duda."
  - "Aplicar el condizionale passato a la subordinada."
tipo: completar  
respuestas_validas:  
  - "non sarei andato"  
  - "Non sarei andato"  
enunciado: "Se ___ (andare) al lavoro, non avrei perso il treno."
```

### 18 — Condizionale passato con verbo reflexivo y auxiliar  
```
metadata:  
  materia: "idiomas-extranjeros/italiano"  
  tema: "condizionale-passato"  
  nivel: "B2"  
  tags: ["reflexivo", "essere"]  
pasos:  
  - "Formar el condizionale passato con verbo reflexivo y auxiliar 'essere'."
  - "Verificar concordancia de género y número."
tipo: completar  
respuestas_validas:  
  - "mi sarei svegliato"  
  - "Mi sarei svegliato"  
enunciado: "Se ___ (svegliarsi) presto, non avrei perso l’appuntamento."
```

### 19 — Condizionale passato en oración de negación con impersonal  
```
metadata:  
  materia: "idiomas-extranjeros/italiano"  
  tema: "condizionale-passato"  
  nivel: "B2"  
  tags: ["negazione", "impersonale"]  
pasos:  
  - "Usar el condizionale passato en oraciones impersonales con negación."
  - "Verificar la forma del verbo auxiliar y participio."
tipo: completar  
respuestas_validas:  
  - "non si sarebbe dovuto"  
  - "Non si sarebbe dovuto"  
enunciado: "Se ___ (dovere) attendere, non saremmo andati."
```

### 20 — Condizionale passato con verbo irregolare y participio  
```
metadata:  
  materia: "idiomas-extranjeros/italiano"  
  tema: "condizionale-passato"  
  nivel: "B2"  
  tags: ["verbo irregular", "participio"]  
pasos:  
  - "Formar el condizionale passato con verbo irregolare y participio."
  - "Verificar la forma del participio."
tipo: completar  
respuestas_validas:  
  - "avrei dovuto"  
  - "Avrei dovuto"  
enunciado: "Se ___ (dovere) studiare, non avrei fallito."
```

### 21 — Condizionale passato en oración de causa con verbo compuesto  
```
metadata:  
  materia: "idiomas-extranjeros/italiano"  
  tema: "condizionale-passato"  
  nivel: "B2"  
  tags: ["causa", "verbo composto"]  
pasos:  
  - "Identificar la relación causal y aplicar el condizionale passato."
  - "Verificar el verbo auxiliar y participio."
tipo: completar  
respuestas_validas:  
  - "sarei andato"  
  - "Sarei andato"  
enunciado: "Se ___ (andare) in tempo, non avremmo perso l’autobus."
```

### 22 — Condizionale passato con verbo impersonale y participio  
```
metadata:  
  materia: "idiomas-extranjeros/italiano"  
  tema: "condizionale-passato"  
  nivel: "B2"  
  tags: ["impersonale", "participio"]  
pasos:  
  - "Usar el condizionale passato en oraciones impersonales."
  - "Verificar la forma del participio y auxiliar."
tipo: completar  
respuestas_validas:  
  - "si sarebbe dovuto"  
  - "Si sarebbe dovuto"  
enunciado: "Se ___ (dovere) aspettare, non saremmo partiti."
```

### 23 — Condizionale passato con verbo reflexivo y participio irregolare  
```
metadata:  
  materia: "idiomas-extranjeros/italiano"  
  tema: "condizionale-passato"  
  nivel: "B2"  
  tags: ["reflexivo", "participio irregular"]  
pasos:  
  - "Formar el condizionale passato con verbo reflexivo y participio irregolare."
  - "Verificar concordancia de género y número."
tipo: completar  
respuestas_validas:  
  - "mi sarei lavato"  
  - "Mi sarei lavato"  
enunciado: "Se ___ (lavarsi), non avrei preso freddo."
```

### 24 — Condizionale passato en oración de negación con verbo irregolare  
```
metadata:  
  materia: "idiomas-extranjeros/italiano"  
  tema: "condizionale-passato"  
  nivel: "B2"  
  tags: ["negazione", "verbo irregular"]  
pasos:  
  - "Analizar la oración de negación con verbo irregolare."
  - "Aplicar el condizionale passato correctamente."
tipo: completar  
respuestas_validas:  
  - "non avrei voluto"  
  - "Non avrei voluto"  
enunciado: "Se ___ (volere) parlare, non l’avrei mai fatto."
```

### 25 — Condizionale passato en oración de duda con verbo compuesto  
```
metadata:  
  materia: "idiomas-extranjeros/italiano"  
  tema: "condizionale-passato"  
  nivel: "B2"  
  tags: ["duda", "verbo composto"]  
pasos:  
  - "Identificar la oración de duda con verbo compuesto."
  - "Aplicar el condizionale passato a la subordinada."
tipo: completar  
respuestas_validas:  
  - "non sarei andato"  
  - "Non sarei andato"  
enunciado: "Se ___ (andare) al lavoro, non avrei perso il treno."```
