# Idiomas — Portugués (Brasil) — subjuntivo-completo (cuestionario)

> Promovido desde el borrador (`../../../_borradores-gemma/idiomas-extranjeros/portugues-br/subjuntivo-completo/cuestionario_crudo.md`)
> con una corrección mecánica: el bloque 10 tenía comillas dobles anidadas dentro de un ítem de
> `pasos:` ya entrecomillado, que cerraban la cadena antes de tiempo; se corrigieron.

---

### 1 — Subjuntivo após "espero que"  
```
metadata:  
  materia: "idiomas-extranjeros/portugues-br"  
  tema: "subjuntivo-completo"  
  nivel: "B2"  
  tags: ["expresiones de deseo", "estructura"]  
pasos:  
  - "Identificar la estructura gramatical que requiere subjuntivo."
  - "Verificar el verbo en la cláusula subordinada."
tipo: completar  
enunciado: "Espero que ele _______ mais atento nas aulas."  
respuestas_validas:  
  - "seja"  
  - "Seja"
```



### 2 — Subjuntivo con "como se"  
```
metadata:  
  materia: "idiomas-extranjeros/portugues-br"  
  tema: "subjuntivo-completo"  
  nivel: "B2"  
  tags: ["expresiones de duda", "ejemplos"]  
pasos:  
  - "Reconocer el uso del subjuntivo tras expresiones de duda."
  - "Ajustar el tiempo verbal según la oración principal."
tipo: completar  
enunciado: "Como se _______ que ele não viria, cancelamos o plano."  
respuestas_validas:  
  - "soubesse"  
  - "Soubesse"
```



### 3 — Subjuntivo en oraciones hipotéticas  
```
metadata:  
  materia: "idiomas-extranjeros/portugues-br"  
  tema: "subjuntivo-completo"  
  nivel: "B2"  
  tags: ["condicionales", "ejemplos"]  
pasos:  
  - "Identificar la oración hipotética que requiere subjuntivo."
  - "Aplicar la conjugación correcta del verbo en subjuntivo."
tipo: completar  
enunciado: "Se eu _______ mais dinheiro, compraria uma casa nova."  
respuestas_validas:  
  - "tivesse"  
  - "Tivesse"
```



### 4 — Subjuntivo con "é preciso que"  
```
metadata:  
  materia: "idiomas-extranjeros/portugues-br"  
  tema: "subjuntivo-completo"  
  nivel: "B2"  
  tags: ["necesidades", "estructura"]  
pasos:  
  - "Reconocer que el subjuntivo se usa tras expresiones de necesidad."
  - "Verificar la conjugación del verbo en la cláusula subordinada."
tipo: completar  
enunciado: "É preciso que você _______ mais para passar no exame."  
respuestas_validas:  
  - "estude"  
  - "Estude"
```



### 5 — Subjuntivo en oraciones negativas  
```
metadata:  
  materia: "idiomas-extranjeros/portugues-br"  
  tema: "subjuntivo-completo"  
  nivel: "B2"  
  tags: ["negación", "ejemplos"]  
pasos:  
  - "Identificar oraciones negativas que requieren subjuntivo."
  - "Ajustar el verbo según el sujeto de la cláusula subordinada."
tipo: completar  
enunciado: "Não acredito que ele _______ capaz de fazer isso sozinho."  
respuestas_validas:  
  - "fosse"  
  - "Fosse"
```



### 6 — Subjuntivo con "prefiro que"  
```
metadata:  
  materia: "idiomas-extranjeros/portugues-br"  
  tema: "subjuntivo-completo"  
  nivel: "B2"  
  tags: ["preferencias", "estructura"]  
pasos:  
  - "Verificar el uso del subjuntivo tras expresiones de preferencia."
  - "Ajustar el verbo según la persona y tiempo."
tipo: completar  
enunciado: "Prefiro que você _______ conosco ao cinema."  
respuestas_validas:  
  - "vá"  
  - "Vá"
```



### 7 — Subjuntivo en oraciones de duda  
```
metadata:  
  materia: "idiomas-extranjeros/portugues-br"  
  tema: "subjuntivo-completo"  
  nivel: "B2"  
  tags: ["dudas", "ejemplos"]  
pasos:  
  - "Identificar oraciones que expresan duda o incertidumbre."
  - "Aplicar el subjuntivo en la cláusula subordinada."
tipo: completar  
enunciado: "Duvido que ele _______ a verdade."  
respuestas_validas:  
  - "soubesse"  
  - "Soubesse"
```



### 8 — Subjuntivo con "é melhor que"  
```
metadata:  
  materia: "idiomas-extranjeros/portugues-br"  
  tema: "subjuntivo-completo"  
  nivel: "B2"  
  tags: ["recomendaciones", "estructura"]  
pasos:  
  - "Reconocer el uso del subjuntivo tras expresiones de recomendación."
  - "Ajustar la conjugación del verbo en la cláusula subordinada."
tipo: completar  
enunciado: "É melhor que você _______ agora mesmo."  
respuestas_validas:  
  - "estude"  
  - "Estude"
```



### 9 — Subjuntivo en oraciones de orden  
```
metadata:  
  materia: "idiomas-extranjeros/portugues-br"  
  tema: "subjuntivo-completo"  
  nivel: "B2"  
  tags: ["órdenes", "ejemplos"]  
pasos:  
  - "Identificar oraciones que expresan órdenes o mandatos."
  - "Aplicar el subjuntivo en la cláusula subordinada."
tipo: completar  
enunciado: "Peço que você _______ suas coisas antes de sair."  
respuestas_validas:  
  - "traga"  
  - "Traga"
```



### 10 — Subjuntivo con "como se" en contexto hipotético  
```
metadata:  
  materia: "idiomas-extranjeros/portugues-br"  
  tema: "subjuntivo-completo"  
  nivel: "B2"  
  tags: ["hipótesis", "estructura"]  
pasos:  
  - "Reconocer la estructura 'como se + subjuntivo'."
  - "Ajustar el verbo según el contexto hipotético."
tipo: completar  
enunciado: "Como se _______ que ele estivesse cansado, não o convidei."  
respuestas_validas:  
  - "tivesse"  
  - "Tivesse"
```



### 11 — Subjuntivo en oraciones de deseo  
```
metadata:  
  materia: "idiomas-extranjeros/portugues-br"  
  tema: "subjuntivo-completo"  
  nivel: "B2"  
  tags: ["deseos", "ejemplos"]  
pasos:  
  - "Identificar oraciones que expresan deseos o anhelos."
  - "Aplicar el subjuntivo en la cláusula subordinada."
tipo: completar  
enunciado: "Quero que você _______ mais responsável."  
respuestas_validas:  
  - "fosse"  
  - "Fosse"
```



### 12 — Subjuntivo con "é necessário que"  
```
metadata:  
  materia: "idiomas-extranjeros/portugues-br"  
  tema: "subjuntivo-completo"  
  nivel: "B2"  
  tags: ["necesidades", "ejemplos"]  
pasos:  
  - "Verificar el uso del subjuntivo tras expresiones de necesidad."
  - "Ajustar la conjugación según el sujeto."
tipo: completar  
enunciado: "É necessário que ela _______ ao médico imediatamente."  
respuestas_validas:  
  - "vá"  
  - "Vá"
```



### 13 — Subjuntivo en oraciones de consejo  
```
metadata:  
  materia: "idiomas-extranjeros/portugues-br"  
  tema: "subjuntivo-completo"  
  nivel: "B2"  
  tags: ["consejos", "estructura"]  
pasos:  
  - "Reconocer oraciones que expresan consejos o sugerencias."
  - "Aplicar el subjuntivo en la cláusula subordinada."
tipo: completar  
enunciado: "Sugiro que você _______ mais para a prova."  
respuestas_validas:  
  - "estude"  
  - "Estude"
```



### 14 — Subjuntivo con "não importa que"  
```
metadata:  
  materia: "idiomas-extranjeros/portugues-br"  
  tema: "subjuntivo-completo"  
  nivel: "B2"  
  tags: ["indiferencia", "ejemplos"]  
pasos:  
  - "Identificar oraciones que expresan indiferencia o neutralidad."
  - "Ajustar el verbo al subjuntivo."
tipo: completar  
enunciado: "Não importa que ele _______ atrasado."  
respuestas_validas:  
  - "seja"  
  - "Seja"
```



### 15 — Subjuntivo en oraciones de duda con "não acredito que"  
```
metadata:  
  materia: "idiomas-extranjeros/portugues-br"  
  tema: "subjuntivo-completo"  
  nivel: "B2"  
  tags: ["negación", "ejemplos"]  
pasos:  
  - "Reconocer el uso del subjuntivo tras expresiones de duda."
  - "Ajustar la conjugación según el contexto."
tipo: completar  
enunciado: "Não acredito que ele _______ sobre o assunto."  
respuestas_validas:  
  - "soubesse"  
  - "Soubesse"
```



### 16 — Subjuntivo con "é melhor que" en oración negativa  
```
metadata:  
  materia: "idiomas-extranjeros/portugues-br"  
  tema: "subjuntivo-completo"  
  nivel: "B2"  
  tags: ["recomendaciones", "ejemplos"]  
pasos:  
  - "Identificar oraciones negativas que requieren subjuntivo."
  - "Aplicar la conjugación correcta del verbo."
tipo: completar  
enunciado: "É melhor que você _______ agora."  
respuestas_validas:  
  - "não estude"  
  - "Não estude"
```



### 17 — Subjuntivo en oraciones de orden con "pedir que"  
```
metadata:  
  materia: "idiomas-extranjeros/portugues-br"  
  tema: "subjuntivo-completo"  
  nivel: "B2"  
  tags: ["órdenes", "ejemplos"]  
pasos:  
  - "Reconocer el uso del subjuntivo tras verbos de petición."
  - "Ajustar la conjugación según el sujeto."
tipo: completar  
enunciado: "Peço que você _______ suas coisas antes."  
respuestas_validas:  
  - "traga"  
  - "Traga"
```



### 18 — Subjuntivo con "como se" en oración negativa  
```
metadata:  
  materia: "idiomas-extranjeros/portugues-br"  
  tema: "subjuntivo-completo"  
  nivel: "B2"  
  tags: ["hipótesis", "ejemplos"]  
pasos:  
  - "Identificar oraciones negativas que usan el subjuntivo."
  - "Ajustar la conjugación del verbo según el contexto."
tipo: completar  
enunciado: "Como se _______ que ele não estivesse presente, não o esperamos."  
respuestas_validas:  
  - "tivesse"  
  - "Tivesse"
```



### 19 — Subjuntivo en oraciones de deseo con "espero que"  
```
metadata:  
  materia: "idiomas-extranjeros/portugues-br"  
  tema: "subjuntivo-completo"  
  nivel: "B2"  
  tags: ["deseos", "ejemplos"]  
pasos:  
  - "Reconocer el uso del subjuntivo tras expresiones de deseo."
  - "Verificar la conjugación según el sujeto."
tipo: completar  
enunciado: "Espero que você _______ mais atento."  
respuestas_validas:  
  - "seja"  
  - "Seja"
```



### 20 — Subjuntivo con "é preciso que" en oración negativa  
```
metadata:  
  materia: "idiomas-extranjeros/portugues-br"  
  tema: "subjuntivo-completo"  
  nivel: "B2"  
  tags: ["necesidades", "ejemplos"]  
pasos:  
  - "Identificar oraciones negativas que requieren subjuntivo."
  - "Ajustar la conjugación del verbo según el contexto."
tipo: completar  
enunciado: "É preciso que você _______ ao trabalho."  
respuestas_validas:  
  - "não vá"  
  - "Não vá"
```



### 21 — Subjuntivo en oraciones de consejo con "sugiro que"  
```
metadata:  
  materia: "idiomas-extranjeros/portugues-br"  
  tema: "subjuntivo-completo"  
  nivel: "B2"  
  tags: ["consejos", "ejemplos"]  
pasos:  
  - "Reconocer el uso del subjuntivo tras verbos de consejo."
  - "Aplicar la conjugación correcta del verbo."
tipo: completar  
enunciado: "Sugiro que você _______ mais antes do exame."  
respuestas_validas:  
  - "estude"  
  - "Estude"
```



### 22 — Subjuntivo con "não acredito que" en oración negativa  
```yaml
metadata:
  materia: "portugues-br"
  tema: "subjuntivo-completo"
  nivel: "B2"
  tags: ["desejo", "subjuntivo", "verbo ser"]
pasos:
  - "Identificar la estructura gramatical que requiere el subjuntivo."
  - "Aplicar la forma correcta del verbo 'ser' en modo subjuntivo presente."
tipo: completar
respuestas_validas:
  - "seja"
  - "Seja"
enunciado: "Quero que ele _______ mais atento às regras."
explicacion: "Después de verbos de deseo como 'quero', se usa el subjuntivo presente. El verbo 'ser' en esta forma es 'seja'."
```

### 23 — Subjuntivo en oraciones de orden con "pedir que" (negativo)  
```
metadata:  
  materia: "idiomas-extranjeros/portugues-br"  
  tema: "subjuntivo-completo"  
  nivel: "B2"  
  tags: ["órdenes", "ejemplos"]  
pasos:  
  - "Identificar oraciones negativas que requieren subjuntivo."
  - "Ajustar la conjugación del verbo según el sujeto."
tipo: completar  
enunciado: "Peço que você _______ suas coisas agora."  
respuestas_validas:  
  - "não traga"  
  - "Não traga"
```



### 24 — Subjuntivo con "como se" en oración de duda  
```
metadata:  
  materia: "idiomas-extranjeros/portugues-br"  
  tema: "subjuntivo-completo"  
  nivel: "B2"  
  tags: ["hipótesis", "ejemplos"]  
pasos:  
  - "Reconocer el uso del subjuntivo tras expresiones de duda."
  - "Aplicar la conjugación correcta del verbo."
tipo: completar  
enunciado: "Como se _______ que ele não fosse, cancelamos tudo."  
respuestas_validas:  
  - "tivesse"  
  - "Tivesse"
```



### 25 — Subjuntivo en oraciones de deseo con "desejo que"  
```
metadata:  
  materia: "idiomas-extranjeros/portugues-br"  
  tema: "subjuntivo-completo"  
  nivel: "B2"  
  tags: ["deseos", "ejemplos"]  
pasos:  
  - "Identificar oraciones que expresan deseos o anhelos."
  - "Aplicar el subjuntivo en la cláusula subordinada."
tipo: completar  
enunciado: "Desejo que você _______ mais responsável."  
respuestas_validas:  
  - "seja"  
  - "Seja"
```
