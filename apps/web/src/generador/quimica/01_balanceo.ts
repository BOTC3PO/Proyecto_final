...
function parseCatalogo(): CatalogItem[] {
  ...
  for (const item of items) {
    ...
    if (item.data.coeficientes.some((coef) => typeof coef !== "number")) {
      throw new Error(`Coeficientes inválidos en catálogo para id=${item.id}.`);
    }

    if (item.data.especies.some((especie) => typeof especie !== "string")) {
...
export function getBalanceoCatalogItemById(itemId: number): CatalogItem {
  ...
  if (!item) {
    throw new Error(
      `No existe itemId=${itemId} en 01_balanceo.enunciados.json para quimica/balanceo.`
    );
  }
  return item;
}
