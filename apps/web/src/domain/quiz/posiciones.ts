 ...
 *
 * Lógica pura y determinística, SIN imports, port byte a byte de
 * `api/src/lib/quiz-posiciones.ts` (misma convención que
 * composition.ts ↔ quiz-composition.ts). Cualquier cambio acá DEBE reflejarse
 * allá o los tipos compartidos se desincronizan.
 *
 ...
 *   - `sorteo.ts` para la selección por dificultad.
 *   - `puntaje.ts` para el peso por variante según dificultad.
 *   - `intentos.ts` (EvaluacionConfig) para la política del docente.
 ...
 *    `indefinido` → factor 1.0 en scoring (ver `puntaje.ts`). El campo es
 ...
 * `apps/web/src/generadoresV2/core/types.ts` pero se redeclaran acá
 * (módulo SIN imports, mirror byte-a-byte con el API).
 */
 ...
 *  en el ruteo (factor 1.0 en el puntaje, ver `puntaje.ts`). El
...
function parseVariante(raw: unknown, indice: number): Variante {
  ...
  // `puntaje.ts`). El coerce centralizado vive en `coerceDificultad` para
  // que la API lo reuse idéntico.
