...
function SlideContent({ slide, cfg }: { slide: Slide; cfg: ThemeConfig }) {
  ...
  const headingCls = hasBg && slide.bgOverlay !== "none"
    ? "text-4xl font-bold leading-tight text-white drop-shadow"
    : cfg.heading;
  const subtitleCls = hasBg && slide.bgOverlay !== "none"
    ? "text-xl font-medium text-white/80 drop-shadow"
    : cfg.subtitle;
  const bodyCls = hasBg && slide.bgOverlay !== "none"
  ...
  if (slide.layout === "split") {
    return (
      <>
        {slide.heading ? <h2 className={headingCls}>{slide.heading}</h2> : null}
        {slide.subtitle ? <p className={subtitleCls}>{slide.subtitle}</p> : null}
        <div className="grid grid-cols-[1fr_1px_1fr] gap-6 flex-1 min-h-0">
          <div className={`overflow-auto whitespace-pre-wrap min-h-0 ${bodyCls}`}>
  ...
  return (
    <>
      ...
      {slide.isCode && slide.body ? (
        ...
        <p className={`${bodyCls} overflow-auto whitespace-pre-wrap flex-1 min-h-0`}>{slide.body}</p>
      ) : null}
    </>
  );
}
    ...
    >
      ...
      <div className={`flex items-center justify-between px-4 py-2 flex-shrink-0 ${cfg.topBar} bg-opacity-90 backdrop-blur-sm`}>
        ...
        <div className="flex items-center gap-3">
          <span className="text-xs text-white/50 tabular-nums">{current + 1} / {slides.length}</span>
          <button
            type="button"
      ...
      <div className="flex-1 flex items-center justify-center p-4 md:p-10 min-h-0">
        <div
          className={`w-full max-w-4xl rounded-2xl overflow-hidden relative transition-opacity duration-200 ${cfg.slide} ${
        ...
        >
          ...
          {/* Content */}
          <div className={`relative z-10 p-8 md:p-12 ${layoutContainerClass(layout)}`}>
          ...
          {slides.map((_, i) => (
            <button
              ...
              className={`w-2 h-2 rounded-full transition-all ${
                i === current ? cfg.dotActive : cfg.dotInactive
              }`}
              onClick={() => goTo(i)}
