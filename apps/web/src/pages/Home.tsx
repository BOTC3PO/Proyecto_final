...
export default function HomePage() {
  ...
  return (
    <div className="flex-grow">
      <div className="my-12">
        <h2 className="mt-8 mb-3 text-4xl sm:text-6xl font-bold text-center text-blue-600 px-4">{t("home.revolucionaElAprendizaje")}</h2>
        <h3 className="mb-5 text-center text">{t("home.aprendeATuRitmoCon")}</h3>
      </div>

      <div className="flex flex-col sm:flex-row w-full gap-3 sm:gap-0 mb-8 justify-evenly px-4 sm:px-0">
        <Link to="/register" className="flex items-center justify-center px-6 py-2 bg-blue-500 rounded hover:bg-blue-600 transition-colors">
          <p className="text-white">{t("home.empiezaComoEstudiante")}</p>
        </Link>
        <Link to="/explorar" className="flex items-center justify-center px-6 py-2 bg-gray-400 rounded hover:bg-gray-500 transition-colors">
          <p>{t("home.exploraComoProfesor")}</p>
        </Link>
      </div>

      <div className="grid grid-cols-2 gap-4 p-8 mt-16 text-center bg-gray-100 md:grid-cols-4 min-h-64">
