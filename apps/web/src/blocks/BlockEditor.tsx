...
import type { BlockDocument, Block, TextBlock, LatexBlock, TableBlock } from "./types"
...
export function BlockEditor({ value, onChange }: Props) {
  ...
  const handleAdd = (type: Block["type"]) => {
    let block: Block
    if (type === "text") {
      block = { id: generateId(), type: "text", content: "" } satisfies TextBlock
    } else if (type === "latex") {
      block = { id: generateId(), type: "latex", content: "", displayMode: true } satisfies LatexBlock
    } else if (type === "table") {
      block = {
        id: generateId(),
        type: "table",
  ...
  return (
    <div className="space-y-3">
              ...
              {block.type === "text" ? (
                ...
                <div className="flex items-center justify-between">
                  <span className="text-xs text-gray-500">
                    Bloque {block.type} (solo lectura en el editor)
                  </span>
                  <button
                    type="button"
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {(["text", "latex", "table"] as const).map((t) => (
          <button
            key={t}
            type="button"
            ...
            onClick={() => handleAdd(t)}
          >
            {t === "text" ? "Texto" : t === "latex" ? "LaTeX" : "Tabla"}
          </button>
        ))}
      </div>
    </div>
