...
encuestas.get("/api/encuestas", requireUser, async (req, res) => {
  ...
  const items = await prisma.encuesta.findMany({
    where: { classroomId: aulaId },
...
encuestas.post("/api/encuestas", requireUser, requireStaff, ...bodyLimitMB(ENV.MAX_PAGE_MB), async (req, res) => {
  try {
    ...
    const result = await prisma.encuesta.create({
      data: { ...parsed, options: JSON.stringify(parsed.options) }
    });
    res.status(201).json({ id: result.id, surveyId: parsed.id });
  } catch (e: any) {
...
encuestas.post("/api/encuestas/:id/votos", requireUser, ...bodyLimitMB(ENV.MAX_PAGE_MB), async (req, res) => {
  ...
  try {
    ...
    const existingVote = await prisma.encuestaRespuesta.findFirst({
      where: { surveyId: survey.id, classroomId: aulaId, usuarioId }
    });
    ...
    if (survey.type === "normal") {
      ...
      await prisma.encuestaRespuesta.create({
        data: {
          surveyId: survey.id,
          classroomId: aulaId,
          usuarioId,
...
encuestas.get("/api/encuestas/:id/resultados", requireUser, async (req, res) => {
  ...
  if (survey.type === "puntuacion") {
    ...
    for (const response of responses) {
      const scores = typeof response.scores === "string"
        ? (JSON.parse(response.scores) as Array<{ optionId: string; score: number }>)
        : [];
  ...
  if (survey.type === "segunda_vuelta") {
    ...
    while (active.length > 0) {
      ...
      for (const response of responses) {
        const ranking = typeof response.ranking === "string"
          ? (JSON.parse(response.ranking) as string[])
          : [];
