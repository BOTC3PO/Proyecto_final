...
VALUES
  ('item-tema-clasico',  'tema', 'Clásico',
   'Diseño limpio y familiar.', 0, 'PF', 'clasico',  1, 1, datetime('now')),
  ('item-tema-nocturno', 'tema', 'Nocturno',
   'Colores oscuros para modo noche.', 0, 'PF', 'nocturno', 1, 3, datetime('now'));
...
VALUES
  ('item-tema-aurora',   'tema', 'Aurora',
   'Paleta suave con gradientes cálidos.', 40, 'PF', 'aurora',   1, 2, datetime('now')),
  ('item-tema-vibrante', 'tema', 'Vibrante',
   'Acentos coloridos para destacar logros.', 65, 'PF', 'vibrante', 1, 4, datetime('now')),
  ('item-tema-bosque',   'tema', 'Bosque',
   'Tonos naturales y verdes.', 40, 'PF', 'bosque',   1, 5, datetime('now')),
  ('item-tema-galaxy',   'tema', 'Galaxy',
   'Fondo animado con gradiente espacial.', 80, 'PF', 'galaxy',   1, 6, datetime('now')),
  ('item-tema-sunset',   'tema', 'Sunset',
   'Degradado cálido al atardecer.', 80, 'PF', 'sunset',   1, 7, datetime('now')),
  ('item-tema-ocean',    'tema', 'Ocean',
   'Profundidades del océano animadas.', 80, 'PF', 'ocean',    1, 8, datetime('now')),
  ('item-tema-candy',    'tema', 'Candy',
   'Colores pastel con navbar animado.', 90, 'PF', 'candy',    1, 9, datetime('now')),
  ('item-tema-neon',     'tema', 'Neon',
   'Negro con efectos neón pulsantes.', 100, 'PF', 'neon',    1, 10, datetime('now'));
...
VALUES
  ('item-avatar-iniciales', 'avatar', 'Iniciales',
   'Avatar con tus iniciales.', 0, 'PF', 'iniciales', 1, 1, datetime('now')),
  ('item-avatar-identicon', 'avatar', 'Identicon',
   'Patrón único generado desde tu ID.', 0, 'PF', 'identicon', 1, 2, datetime('now'));
